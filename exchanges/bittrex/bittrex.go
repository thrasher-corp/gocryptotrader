...
func (b *Bittrex) GetMarkets() (Market, error) {
	...
	if err := b.SendHTTPRequest(defaultRest, path, &markets); err != nil {
		return markets, err
	}
...
func (b *Bittrex) GetCurrencies() (Currency, error) {
	...
	if err := b.SendHTTPRequest(defaultRest, path, &currencies); err != nil {
...
func (b *Bittrex) GetTicker(currencyPair string) (Ticker, error) {
	tick := Ticker{}
	path := fmt.Sprintf("/%s?market=%s", bittrexAPIGetTicker,
...
func (b *Bittrex) GetMarketSummaries() (MarketSummary, error) {
	...
	path := fmt.Sprintf("/%s/", bittrexAPIGetMarketSummaries)

	if err := b.SendHTTPRequest(defaultRest, path, &summaries); err != nil {
...
func (b *Bittrex) GetMarketSummary(currencyPair string) (MarketSummary, error) {
	...
	path := fmt.Sprintf("/%s?market=%s", bittrexAPIGetMarketSummary, strings.ToLower(currencyPair))

	if err := b.SendHTTPRequest(defaultRest, path, &summary); err != nil {
		return summary, err
	}
...
func (b *Bittrex) GetOrderbook(currencyPair string) (OrderBooks, error) {
	...
	path := fmt.Sprintf("/%s?market=%s&type=both&depth=50", bittrexAPIGetOrderbook, strings.ToUpper(currencyPair))
...
func (b *Bittrex) GetMarketHistory(currencyPair string) (MarketHistory, error) {
	...
	path := fmt.Sprintf("/%s?market=%s", bittrexAPIGetMarketHistory, strings.ToUpper(currencyPair))
...
func (b *Bittrex) PlaceBuyLimit(currencyPair string, quantity, rate float64) (UUID, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPIBuyLimit)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, values, &id); err != nil {
		return id, err
	}
...
func (b *Bittrex) PlaceSellLimit(currencyPair string, quantity, rate float64) (UUID, error) {
	...
	values.Set("quantity", strconv.FormatFloat(quantity, 'E', -1, 64))
	values.Set("rate", strconv.FormatFloat(rate, 'E', -1, 64))
	path := fmt.Sprintf("/%s", bittrexAPISellLimit)
...
func (b *Bittrex) GetOpenOrders(currencyPair string) (Order, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPIGetOpenOrders)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, values, &orders); err != nil {
		return orders, err
	}
...
func (b *Bittrex) CancelExistingOrder(uuid string) (Balances, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPICancel)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, values, &balances); err != nil {
		return balances, err
	}
...
func (b *Bittrex) GetAccountBalances() (Balances, error) {
	var balances Balances
	path := fmt.Sprintf("/%s", bittrexAPIGetBalances)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, url.Values{}, &balances); err != nil {
		return balances, err
	}
...
func (b *Bittrex) GetAccountBalanceByCurrency(currency string) (Balance, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPIGetBalance)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, values, &balance); err != nil {
		return balance, err
	}
...
func (b *Bittrex) GetCryptoDepositAddress(currency string) (DepositAddress, error) {
	...
	values := url.Values{}
	values.Set("currency", currency)
	path := fmt.Sprintf("/%s", bittrexAPIGetDepositAddress)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, values, &address); err != nil {
		return address, err
	}
...
func (b *Bittrex) Withdraw(currency, paymentID, address string, quantity float64) (UUID, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPIWithdraw)
...
func (b *Bittrex) GetOrder(uuid string) (Order, error) {
	...
	values := url.Values{}
	values.Set("uuid", uuid)
	path := fmt.Sprintf("/%s", bittrexAPIGetOrder)

	if err := b.SendAuthenticatedHTTPRequest(defaultRest, path, values, &order); err != nil {
		return order, err
	}
...
func (b *Bittrex) GetOrderHistoryForCurrency(currencyPair string) (Order, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPIGetOrderHistory)
...
func (b *Bittrex) GetWithdrawalHistory(currency string) (WithdrawalHistory, error) {
	...
	path := fmt.Sprintf("/%s", bittrexAPIGetWithdrawalHistory)
