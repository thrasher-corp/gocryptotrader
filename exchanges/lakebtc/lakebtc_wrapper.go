...
// initial operations
func (l *LakeBTC) GetPlatformHistory(p currency.Pair, assetType assets.AssetType, timestampStart time.Time, tradeID string) ([]exchange.PlatformTrade, error) {
	var resp []exchange.PlatformTrade

	t, err := l.GetTradeHistory(p.String())
	if err != nil {
		return resp, err
	}
