...
func (l *LakeBTC) Trade(orderType int, amount, price decimal.Decimal, currency string) (Trade, error) {
	resp := Trade{}
	params := price.StringFixed(exchange.DefaultDecimalPrecision) + "," + amount.StringFixed(exchange.DefaultDecimalPrecision) + "," + currency
...
func (l *LakeBTC) CreateWithdraw(amount decimal.Decimal, accountID int64) (Withdraw, error) {
	resp := Withdraw{}
	params := amount.StringFixed(exchange.DefaultDecimalPrecision) + ",btc," + strconv.FormatInt(accountID, 10)
