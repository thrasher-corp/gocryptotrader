...
func (c *PositionController) SetCollateralCurrency(exch string, item asset.Item, pair currency.Pair, collateralCurrency currency.Code) error {
	...
	tracker, ok := c.multiPositionTrackers[exch][item][pair.Base.Item][pair.Quote.Item]
	if !ok || tracker == nil {
...
func (c *PositionController) TrackFundingDetails(d *FundingRates) error {
	...
	c.m.Lock()
	defer c.m.Unlock()
	tracker, ok := c.multiPositionTrackers[d.Exchange][d.Asset][d.Pair.Base.Item][d.Pair.Quote.Item]
	if !ok || tracker == nil {
