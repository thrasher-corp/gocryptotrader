...
func SetupPositionController() PositionController {
	return PositionController{
		multiPositionTrackers: make(map[key.ExchangePairAssetKey]*MultiPositionTracker),
	}
}
...
func (c *PositionController) TrackNewOrder(d *Detail) error {
	...
	c.m.Lock()
	defer c.m.Unlock()
	exchMap, ok := c.multiPositionTrackers[key.ExchangePairAssetKey{
		Exchange: d.Exchange,
...
func (c *PositionController) SetCollateralCurrency(exch string, item asset.Item, pair currency.Pair, collateralCurrency currency.Code) error {
	...
	tracker := c.multiPositionTrackers[key.ExchangePairAssetKey{
		Exchange: exch,
		Base:     pair.Base.Item,
