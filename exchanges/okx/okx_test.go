...
func TestGetHistoricCandlesExtended(t *testing.T) {
	t.Parallel()
	if _, err := ok.GetHistoricCandlesExtended(context.Background(), currency.Pair{Base: currency.BTC, Quote: currency.NewCode("USD-220930"), Delimiter: currency.DashDelimiter}, asset.Futures, time.Now().Add(-time.Hour*24), time.Now(), kline.OneMin); err != nil {
...
func TestGetRFQTrades(t *testing.T) {
	...
	if _, err := ok.GetRFQTrades(context.Background(), &RFQTradesRequestParams{}); err != nil {
...
func TestGetLightningDeposits(t *testing.T) {
	...
	if _, err = ok.GetLightningDeposits(context.Background(), "BTC", 1.00, 0); err != nil {
...
func TestSavingsPurchase(t *testing.T) {
	...
	if _, err := ok.SavingsPurchaseOrRedemption(context.Background(), &SavingsPurchaseRedemptionInput{
		...
		Rate:       1,
		ActionType: "purchase",
	}); err != nil && !strings.EqualFold(err.Error(), "Insufficient balance") {
...
func TestGetMaximumAvailableTradableAmount(t *testing.T) {
	...
	if _, err := ok.GetMaximumAvailableTradableAmount(context.Background(), "BTC-USDT", "BTC", "cross", true, 123); err != nil {
...
func TestGetMaximumLoanOfInstrument(t *testing.T) {
	...
	if _, err := ok.GetMaximumLoanOfInstrument(context.Background(), "ZRX-BTC", "isolated", "ZRX"); err != nil {
...
func TestGetAccountRiskState(t *testing.T) {
	...
	if _, err := ok.GetAccountRiskState(context.Background()); err != nil {
...
func TestGetBorrowInterestAndLimit(t *testing.T) {
	...
	if _, err := ok.GetBorrowInterestAndLimit(context.Background(), 1, "BTC"); err != nil {
...
func TestGetGreeks(t *testing.T) {
	...
	if _, err := ok.GetGreeks(context.Background(), ""); err != nil {
...
func TestGetSubaccountTradingBalance(t *testing.T) {
	...
	if _, err := ok.GetSubaccountTradingBalance(context.Background(), "test1"); err != nil {
...
func TestGetSubaccountFundingBalance(t *testing.T) {
	...
	if _, err := ok.GetSubaccountFundingBalance(context.Background(), "test1", ""); err != nil && !strings.Contains(err.Error(), "\"msg\":\"Not Found\"") {
...
func TestFetchTradablePairs(t *testing.T) {
	t.Parallel()
	if _, err := ok.FetchTradablePairs(context.Background(), asset.Spot); err != nil {
...
func TestMarkPriceSubscription(t *testing.T) {
	t.Parallel()
	setupWsAuth(t)
	if _, err := ok.MarkPriceSubscription("subscribe", asset.Futures, currency.NewPair(currency.BTC, currency.NewCode("USD-220930"))); err != nil {
...
func TestOrderBooksSubscription(t *testing.T) {
	t.Parallel()
	setupWsAuth(t)
	if _, err := ok.OrderBooksSubscription("subscribe", okxChannelOrderBooks, asset.Futures, currency.NewPair(currency.BTC, currency.NewCode("USD-220930"))); err != nil {
		t.Errorf("%s OrderBooksSubscription() error: %v", ok.Name, err)
	}
	if _, err := ok.OrderBooksSubscription("unsubscribe", okxChannelOrderBooks, asset.Futures, currency.NewPair(currency.BTC, currency.NewCode("USD-220930"))); err != nil {
		t.Errorf("%s OrderBooksSubscription() error: %v", ok.Name, err)
	}
}
