	...
	e.Features = exchange.Features{
		...
		Enabled: exchange.FeaturesEnabled{
			...
			Kline: kline.ExchangeCapabilitiesEnabled{
				...
				// Spread candle history caps at 100 and this limit is shared across assets.
				GlobalResultLimit: 100, // https://www.okx.com/docs-v5/en/#spread-trading-rest-api-get-candlesticks-history
			},
		},
	...
	case asset.Spot, asset.PerpetualSwap, asset.Futures, asset.Options, asset.Margin:
		...
		for y := range ticks {
			...
			for i := range enabledPairs {
				pairFmt, err := e.FormatExchangeCurrency(enabledPairs[i], assetType)
				if err != nil {
					return err
				}
...
func (e *Exchange) UpdateOrderbook(ctx context.Context, pair currency.Pair, assetType asset.Item) (*orderbook.Book, error) {
	...
	case asset.Spot, asset.Options, asset.Margin, asset.PerpetualSwap, asset.Futures:
		...
		book.LastPushed = orderBookD.GenerationTimestamp.Time()
...
func (e *Exchange) CancelBatchOrders(ctx context.Context, o []order.Cancel) (*order.CancelBatchResponse, error) {
	...
	if len(cancelAlgoOrderParams) > 0 {
		...
		if err != nil {
			...
			statusErr := getStatusError(cancelationResponse.StatusCode, cancelationResponse.StatusMessage)
...
func (e *Exchange) GetHistoricCandles(ctx context.Context, pair currency.Pair, a asset.Item, interval kline.Interval, start, end time.Time) (*kline.Item, error) {
	...
	signedResultLimit := int64(resultLimit)
	switch a {
	case asset.Spread:
