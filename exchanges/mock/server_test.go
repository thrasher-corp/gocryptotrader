...
func TestMatchAndGetResponseJSONSlice(t *testing.T) {
	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			got, err := MatchAndGetResponseJSONSlice(mockData, tc.request)
			if tc.wantErr {
...
func TestJSONBodyArrayRegression(t *testing.T) {
	...
	got, err := MatchAndGetResponseJSONSlice(mockData, reqVals)
	require.NoError(t, err)
...
func TestMessageWriteJSON(t *testing.T) {
	...
	assert.Equal(t, applicationJSON, w.Header().Get(contentType), "Content-Type should be set") //nolint:testifylint //false positive; assert.JSONEq is not applicable for Content-Type header comparison
