...
func TestStoreAssetPairStore(t *testing.T) {
	b := Base{
		Config: &config.Exchange{Name: "kitties"},
	}

	err := b.StoreAssetPairStore(asset.Empty, currency.PairStore{})
	assert.ErrorIs(t, err, asset.ErrInvalidAsset)

	err = b.StoreAssetPairStore(asset.Spot, currency.PairStore{})
	assert.ErrorIs(t, err, currency.ErrPairFormatIsNil)

	err = b.StoreAssetPairStore(asset.Spot, currency.PairStore{RequestFormat: &currency.PairFormat{Uppercase: true}})
	assert.ErrorIs(t, err, currency.ErrPairFormatIsNil)
	...
	err = b.StoreAssetPairStore(asset.Futures, currency.PairStore{
		RequestFormat: &currency.PairFormat{Uppercase: true},
		ConfigFormat:  &currency.PairFormat{Uppercase: true, Delimiter: currency.DashDelimiter}})
	assert.NoError(t, err)
	assert.False(t, b.CurrencyPairs.Pairs[asset.Futures].AssetEnabled, "StoreAssetPairStore should not magically enable AssetTypes")
