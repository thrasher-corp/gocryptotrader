...
func TestWebsocketSubmitOrders(t *testing.T) {
	...
	_, err := g.WebsocketSubmitOrders(t.Context(), nil)
	require.ErrorIs(t, err, asset.ErrNotSupported)
	...
	_, err = g.WebsocketSubmitOrders(t.Context(), []*order.Submit{sub})
	require.ErrorIs(t, err, order.ErrPairIsEmpty)

	sub.Pair = currency.NewBTCUSD()
	cpy := *sub
	cpy.AssetType = asset.Futures
	_, err = g.WebsocketSubmitOrders(t.Context(), []*order.Submit{sub, &cpy})
