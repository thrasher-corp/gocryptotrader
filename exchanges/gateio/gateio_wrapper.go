...
// GetMarginRatesHistory returns margin borrow rates and optional borrow costs from loan history.
func (e *Exchange) GetMarginRatesHistory(ctx context.Context, req *margin.RateHistoryRequest) (*margin.RateHistoryResponse, error) {
	...
	statuses := []string{statusOpen, statusLoaned, statusFinished, "auto_repair"}
	resp := &margin.RateHistoryResponse{
		Rates: make([]margin.Rate, 0, 1024),
	}
	seen := make(map[string]struct{})
	for _, status := range statuses {
		for page := uint64(1); ; page++ {
			loans, err := e.GetMarginAllLoans(ctx, status, sideBorrow, "create_time", req.Currency, req.Pair, true, page, pageSize)
			if err != nil {
				return nil, err
			}
			if len(loans) == 0 {
			...
			for i := range loans {
				ts := loans[i].CreateTime.Time()
				if !req.StartDate.IsZero() && ts.Before(req.StartDate) {
					continue
				}
				if !req.EndDate.IsZero() && ts.After(req.EndDate) {
					continue
				}
				if !strings.EqualFold(loans[i].Currency, req.Currency.String()) {
					continue
				}
				if req.Pair.IsPopulated() && !strings.EqualFold(loans[i].CurrencyPair, req.Pair.String()) {
					continue
				}
				if _, ok := seen[loans[i].ID]; ok {
					continue
				}
				seen[loans[i].ID] = struct{}{}
				dailyBorrowRate := loans[i].Rate.Decimal()
				...
				entry := margin.Rate{
					...
					BorrowCost: margin.BorrowCost{
						Cost: loans[i].PaidInterest.Decimal().Add(loans[i].UnpaidInterest.Decimal()),
						Size: loans[i].Amount.Decimal(),
					},
				}
