...
type Ticker struct {
	...
	EtfPreTimestamp  Time         `json:"etf_pre_timestamp"`
...
type OrderbookData struct {
	ID      int64       `json:"id"`
	Current Time        `json:"current"` // The timestamp of the response data being generated (in milliseconds)
	Update  Time        `json:"update"`  // The timestamp of when the orderbook last changed (in milliseconds)
...
type Trade struct {
	...
	TradingTime  Time         `json:"create_time"`
...
type FuturesContract struct {
	...
	FundingNextApply      Time         `json:"funding_next_apply"`
	ConfigChangeTime      Time         `json:"config_change_time"`
...
type OptionUnderlying struct {
	Name       string       `json:"name"`
	IndexPrice types.Number `json:"index_price"`
	IndexTime  Time         `json:"index_time"`
}
...
type ContractClosePosition struct {
	PositionCloseTime Time         `json:"time"`
...
type SpotOrder struct {
	...
	CreateTimeMs       Time         `json:"create_time_ms,omitempty"`
	UpdateTime         Time         `json:"update_time,omitempty"`
	UpdateTimeMs       Time         `json:"update_time_ms,omitempty"`
	CurrencyPair       string       `json:"currency_pair,omitempty"`
...
type TriggerTimeResponse struct {
	TriggerTime Time `json:"trigger_time"`
}
...
type SpotPriceTriggeredOrder struct {
	...
	CreationTime Time             `json:"ctime"`
	FireTime     Time             `json:"ftime"`
...
type WsSpotBalance struct {
	Timestamp   Time         `json:"timestamp"`
	TimestampMs Time         `json:"timestamp_ms"`
...
type WsFuturesOrder struct {
	...
	FinishAs     string        `json:"finish_as"`
	FinishTime   Time          `json:"finish_time"`
	FinishTimeMs Time          `json:"finish_time_ms"`
	Iceberg      int64         `json:"iceberg"`
	ID           int64         `json:"id"`
	IsClose      bool          `json:"is_close"`
	IsLiq        bool          `json:"is_liq"`
...
type WsOptionsUnderlyingPrice struct {
	Underlying   string  `json:"underlying"`
	Price        float64 `json:"price"`
	UpdateTime   Time    `json:"time"`
	UpdateTimeMs Time    `json:"time_ms"`
}
...
type WsOptionsOrder struct {
	...
	Underlying     string        `json:"underlying"`
	User           string        `json:"user"`
	CreationTime   Time          `json:"time"`
	CreationTimeMs Time          `json:"time_ms"`
}
...
type WsOptionsUserSettlement struct {
	...
	StrikePrice  float64 `json:"strike_price"`
	Underlying   string  `json:"underlying"`
	SettleTime   Time    `json:"time"`
	SettleTimeMs Time    `json:"time_ms"`
}
