...
func TestGetTimestamp(t *testing.T) {
	...
	require.NoError(t, err)
	assert.NotEmpty(t, st, "GetCurrentServerTime must return a time")
}
...
func TestWrapperGetServerTime(t *testing.T) {
	...
	require.NoError(t, err)
	assert.NotEmpty(t, st, "GetServerTime must return a time")
}
...
func TestModifyOrder(t *testing.T) {
	...
	require.Error(t, err, "ModifyOrder should error without any order details")
}
...
func TestWsTradeDetail(t *testing.T) {
	...
	close(h.Websocket.DataHandler)
	require.Empty(t, h.Websocket.DataHandler, "Should not see any errors going to datahandler")
}
...
func TestWsOrderUpdate(t *testing.T) {
	...
	require.Len(t, h.Websocket.DataHandler, 4, "Should see correct number of records")
	exp := []*order.Detail{
		{
			Exchange:      h.Name,
			Pair:          btcusdtPair,
...
func TestGetErrResp(t *testing.T) {
	...
	assert.NoError(t, err, "V1 success must not error")

	err = getErrResp([]byte(`{"action":"sub","code":2001,"ch":"naughty.drop🐻s.locally","message":"invalid.ch"}`))
	assert.ErrorContains(t, err, "invalid.ch (2001)", "V2 errors must return correctly")

	err = getErrResp([]byte(`{"action":"sub","code":200,"ch":"orders#btcusdt","data":{}}`))
	assert.NoError(t, err, "V2 success must not error")
}
