...
func (c *Coinbene) GetSwapTickers() (SwapTickers, error) {
	...
	err := c.SendHTTPRequest(exchange.Swap, path, contractTickers, &r)
	if err != nil {
		return nil, err
	}
	return r.Data, nil
...
func (c *Coinbene) GetSwapOrderbook(symbol string, size int64) (Orderbook, error) {
	...
	err := c.SendHTTPRequest(exchange.Swap, path, contractOrderbook, &r)
	if err != nil {
		return s, err
	}
...
func (c *Coinbene) GetSwapKlines(symbol string, start, end time.Time, resolution string) (resp CandleResponse, err error) {
	...
	if err = c.SendHTTPRequest(exchange.Swap, path, contractKline, &resp); err != nil {
		return
	}
...
func (c *Coinbene) GetSwapTrades(symbol string, limit int) (SwapTrades, error) {
	...
	if err := c.SendHTTPRequest(exchange.Swap, path, contractTrades, &r); err != nil {
		return nil, err
	}
...
func (c *Coinbene) GetSwapAccountInfo() (SwapAccountInfo, error) {
	...
	err := c.SendAuthHTTPRequest(exchange.Swap, http.MethodGet,
		path,
		coinbeneAccountInfo,
		true,
		nil,
...
func (c *Coinbene) PlaceSwapOrder(symbol, direction, orderType, marginMode,
	...
	var r resp
	path := coinbeneAPIVersion + coinbenePlaceOrder
	err := c.SendAuthHTTPRequest(exchange.Swap, http.MethodPost,
		path,
		coinbenePlaceOrder,
		true,
		v,
