	...
	for i := range wsTickers {
		for j := range wsTickers[i].Tickers {
			...
			t := ticker.Price{
				...
				ExchangeName: e.Name,
				High:         wsTickers[i].Tickers[j].High24Hour.Float64(),
				Low:          wsTickers[i].Tickers[j].Low24Hour.Float64(),
				Last:         wsTickers[i].Tickers[j].Price.Float64(),
				BaseVolume:   wsTickers[i].Tickers[j].Volume24Hour.Float64(),
	...
	for i := range wsUser {
		for j := range wsUser[i].Orders {
			...
			allOrders = append(allOrders, order.Detail{
				...
				ExecutedQuoteAmount:  wsUser[i].Orders[j].FilledValue.Float64(),
				AverageExecutedPrice: wsUser[i].Orders[j].AveragePrice.Float64(),
