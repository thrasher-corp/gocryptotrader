...
func (i *ItBit) GetTicker(currencyPair string) (Ticker, error) {
	...
	return response, i.SendHTTPRequest(exchange.DefaultSpot, path, &response)
}
...
func (i *ItBit) GetWallets(params url.Values) ([]Wallet, error) {
	...
	return resp, i.SendAuthenticatedHTTPRequest(exchange.DefaultSpot, http.MethodGet, path, nil, &resp)
}
...
func (i *ItBit) CreateWallet(walletName string) (Wallet, error) {
	...
	err := i.SendAuthenticatedHTTPRequest(exchange.DefaultSpot, http.MethodPost, "/"+itbitWallets, params, &resp)
	if err != nil {
		return resp, err
	}
	if resp.Description != "" {
...
func (i *ItBit) GetWallet(walletID string) (Wallet, error) {
	...
	err := i.SendAuthenticatedHTTPRequest(exchange.DefaultSpot, http.MethodGet, path, nil, &resp)
	if err != nil {
		return resp, err
	}
	if resp.Description != "" {
...
func (i *ItBit) GetOrders(walletID, symbol, status string, page, perPage int64) ([]Order, error) {
	...
	return resp, i.SendAuthenticatedHTTPRequest(exchange.DefaultSpot, http.MethodGet, itbitOrders, params, &resp)
}
...
func (i *ItBit) PlaceOrder(walletID, side, orderType, currency string, amount, price float64, instrument, clientRef string) (Order, error) {
	...
	err := i.SendAuthenticatedHTTPRequest(exchange.DefaultSpot, http.MethodPost, path, params, &resp)
	if err != nil {
		return resp, err
	}
	if resp.Description != "" {
...
func (i *ItBit) CancelExistingOrder(walletID, orderID string) error {
	...
	return i.SendAuthenticatedHTTPRequest(exchange.DefaultSpot, http.MethodDelete, path, nil, nil)
}
...
func (i *ItBit) SendHTTPRequest(ep, path string, result interface{}) error {
	endpoint, err := i.API.Endpoints.Get(ep)
	if err != nil {
		return err
	}
...
func (i *ItBit) SendAuthenticatedHTTPRequest(ep, method, path string, params map[string]interface{}, result interface{}) error {
