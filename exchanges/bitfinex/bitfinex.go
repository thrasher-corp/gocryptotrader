...
func (b *Bitfinex) GetMarginPairs() ([]string, error) {
	...
	err := b.SendHTTPRequest(spotURL, path, &resp, status)
	if err != nil {
		return nil, err
	}
	if len(resp) != 1 {
...
func (b *Bitfinex) GetDerivativeData(keys, startTime, endTime string, sort, limit int64) (DerivativeDataResponse, error) {
	...
	path := bitfinexAPIVersion2 + bitfinexDerivativeData +
		params.Encode()
	err := b.SendHTTPRequest(spotURL, path, &result, status)
	if err != nil {
		return response, err
	}
	if len(result) < 1 {
...
func (b *Bitfinex) GetTickerBatch() (map[string]Ticker, error) {
	...
	err := b.SendHTTPRequest(spotURL, path, &response, tickerBatch)
	if err != nil {
		return nil, err
	}
...
func (b *Bitfinex) GetTicker(symbol string) (Ticker, error) {
	...
	err := b.SendHTTPRequest(spotURL, path, &response, tickerFunction)
	if err != nil {
		return Ticker{}, err
	}
...
func (b *Bitfinex) GetTrades(currencyPair string, limit, timestampStart, timestampEnd int64, reOrderResp bool) ([]Trade, error) {
	...
	var resp [][]interface{}
	err := b.SendHTTPRequest(spotURL, path, &resp, tradeRateLimit)
	if err != nil {
		return nil, err
	}
...
func (b *Bitfinex) GetOrderbook(symbol, precision string, limit int64) (Orderbook, error) {
	...
	var response [][]interface{}
	err := b.SendHTTPRequest(spotURL, path, &response, orderbookFunction)
	if err != nil {
...
func (b *Bitfinex) GetStats(symbol string) ([]Stat, error) {
	...
	return response, b.SendHTTPRequest(spotURL, path, &response, statsV1)
}
...
func (b *Bitfinex) GetFundingBook(symbol string) (FundingBook, error) {
	...
	if err := b.SendHTTPRequest(spotURL, path, &response, fundingbook); err != nil {
		return response, err
	}
...
func (b *Bitfinex) GetLends(symbol string, values url.Values) ([]Lends, error) {
	...
	path := common.EncodeURLValues(bitfinexAPIVersion+
		...
		symbol,
		values)
	return response, b.SendHTTPRequest(spotURL, path, &response, lends)
}
...
func (b *Bitfinex) GetCandles(symbol, timeFrame string, start, end int64, limit uint32, historic bool) ([]Candle, error) {
	...
	if historic {
		...
		var response [][]interface{}
		err := b.SendHTTPRequest(spotURL, path, &response, candle)
		if err != nil {
			return nil, err
		}
...
func (b *Bitfinex) GetLeaderboard(key, timeframe, symbol string, sort, limit int, start, end string) ([]LeaderboardEntry, error) {
	...
	var resp []interface{}
	if err := b.SendHTTPRequest(spotURL, path, &resp, leaderBoardReqRate); err != nil {
		return nil, err
	}
...
func (b *Bitfinex) GetAccountFees() ([]AccountInfo, error) {
	...
	return responses, b.SendAuthenticatedHTTPRequest(spotURL, http.MethodPost,
		bitfinexAccountInfo,
		nil,
		&responses,
		getAccountFees)
...
func (b *Bitfinex) GetWithdrawalFees() (AccountFees, error) {
	...
	return response, b.SendAuthenticatedHTTPRequest(spotURL, http.MethodPost,
		bitfinexAccountFees,
		nil,
		&response,
		getWithdrawalFees)
...
func (b *Bitfinex) WalletTransfer(amount float64, currency, walletFrom, walletTo string) (WalletTransfer, error) {
	...
	err := b.SendAuthenticatedHTTPRequest(spotURL, http.MethodPost,
		bitfinexTransfer,
		req,
		&response,
		walletTransfer)
...
func (b *Bitfinex) CancelMultipleOrders(orderIDs []int64) (string, error) {
	...
	return response.Result, b.SendAuthenticatedHTTPRequest(spotURL, http.MethodPost,
...
func (b *Bitfinex) GetOrderStatus(orderID int64) (Order, error) {
	...
	return orderStatus, b.SendAuthenticatedHTTPRequest(spotURL, http.MethodPost,
		bitfinexOrderStatus,
		req,
		&orderStatus,
		orderMulti)
