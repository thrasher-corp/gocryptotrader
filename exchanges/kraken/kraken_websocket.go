...
func (k *Kraken) wsProcessOrderBookPartial(pair currency.Pair, obSnapshot *wsSnapshot, levels int) error {
	base := orderbook.Base{
		...
		Bids:                   make(orderbook.Tranches, len(obSnapshot.Bids)),
		Asks:                   make(orderbook.Tranches, len(obSnapshot.Asks)),
		MaxDepth:               levels,
...
func (k *Kraken) wsProcessOrderBookUpdate(pair currency.Pair, wsUpdt *wsUpdate) error {
	obUpdate := orderbook.Update{
		Asset: asset.Spot,
		Pair:  pair,
		Bids:  make([]orderbook.Tranche, len(wsUpdt.Bids)),
		Asks:  make([]orderbook.Tranche, len(wsUpdt.Asks)),
	}
