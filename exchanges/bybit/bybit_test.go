		...
		for _, curr := range []currency.Code{currency.USDC, currency.AVAX, currency.USDT} {
			t.Run(curr.String(), func(t *testing.T) {
				...
				case currency.USDC:
					...
					assert.Equal(t, -30723.63021638, bal.Hold, "Hold amount should be correct")
					assert.Equal(t, 30723.630216383714, bal.Borrowed, "Borrowed amount should be correct")
					assert.Equal(t, 0.0, bal.Free, "Free amount should be correct")
				case currency.AVAX:
					assert.Equal(t, 2473.9, bal.Total, "Total amount should be correct")
					assert.Equal(t, 1468.10808813, bal.Hold, "Hold amount should be correct")
					assert.Equal(t, 0.0, bal.Borrowed, "Borrowed amount should be correct")
					assert.Equal(t, 1005.79191187, bal.Free, "Free amount should be correct")
				case currency.USDT:
					assert.Equal(t, 935.1415, bal.Total, "Total amount should be correct")
					assert.Equal(t, 0.0, bal.Borrowed, "Borrowed amount should be correct")
					assert.Equal(t, 0.0, bal.Hold, "Hold amount should be correct")
