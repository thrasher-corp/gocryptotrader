...
func (h *HUOBIHADAX) BuildHuobiWebsocketRequest(msgType string, requestIndex int64, symbolRequest []string) map[string]interface{} {
...
func (h *HUOBIHADAX) BuildHuobiWebsocketRequestExtra(msgType string, requestIndex int64, symbolIDList interface{}) interface{} {
	request := map[string]interface{}{}
	request["version"] = 1
	request["msgType"] = msgType
...
func (h *HUOBIHADAX) BuildHuobiWebsocketParamsList(objectName, currency, pushType, period, count, from, to, percentage string) interface{} {
...
func (h *HUOBIHADAX) OnConnect(output chan socketio.Message) {
	if h.Verbose {
...
func (h *HUOBIHADAX) OnDisconnect(output chan socketio.Message) {
	log.Printf("%s Disconnected from websocket server.. Reconnecting.\n", h.GetName())
	h.WebsocketClient()
}

// OnError handles error issues
func (h *HUOBIHADAX) OnError() {
	log.Printf("%s Error with Websocket connection.. Reconnecting.\n", h.GetName())
	h.WebsocketClient()
}

// OnMessage handles messages from the exchange
func (h *HUOBIHADAX) OnMessage(message []byte, output chan socketio.Message) {
}

// OnRequest handles requests
func (h *HUOBIHADAX) OnRequest(message []byte, output chan socketio.Message) {
...
func (h *HUOBIHADAX) WebsocketClient() {
