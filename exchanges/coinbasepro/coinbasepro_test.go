...
func TestWsHandleData(t *testing.T) {
	...
	assert.Truef(t, errors.As(err, &syntaxErr) || strings.Contains(err.Error(), "Syntax error no sources available, the input json is empty"), errJSONUnmarshalUnexpected)
	...
	_, err = c.wsHandleData(mockJSON)
	assert.Truef(t, errors.As(err, &unmarshalTypeErr) || strings.Contains(err.Error(), "mismatched type with value"), errJSONUnmarshalUnexpected)
