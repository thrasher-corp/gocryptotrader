...
func (c *CoinbasePro) GetHistoricRates(currencyPair, startTime, endTime string, granularity int64) ([]History, error) {
	var resp [][]interface{}
	history := []History{}
	values := url.Values{}

	if startTime != "" {
		values.Set("start", startTime)
	}

	if endTime != "" {
		values.Set("end", endTime)
	}
