...
func (e *Exchange) processUserOrders(respRaw []byte, channels []string) error {
	...
	for x := range orderData {
		a, cp, err := e.getAssetPairByInstrument(orderData[x].InstrumentName)
		if err != nil {
			return err
		}
...
func (e *Exchange) processUserOrderChanges(respRaw []byte, channels []string) error {
	...
	for x := range changeData.Trades {
		...
		var a asset.Item
		a, cp, err = e.getAssetPairByInstrument(changeData.Trades[x].InstrumentName)
		if err != nil {
			return err
		}
	...
	for x := range orders {
		...
		a, cp, err := e.getAssetPairByInstrument(changeData.Orders[x].InstrumentName)
		if err != nil {
			return err
		}
...
func (e *Exchange) processQuoteTicker(respRaw []byte, channels []string) error {
	a, cp, err := e.getAssetPairByInstrument(channels[1])
	if err != nil {
		return err
	}
	var response WsResponse
...
func (e *Exchange) processTrades(respRaw []byte, channels []string) error {
	...
	for x := range tradesData {
		var cp currency.Pair
		var a asset.Item
		a, cp, err = e.getAssetPairByInstrument(tradeList[x].InstrumentName)
		if err != nil {
			return err
		}
...
func (e *Exchange) processCandleChart(respRaw []byte, channels []string) error {
	...
	a, cp, err := e.getAssetPairByInstrument(channels[2])
	if err != nil {
		return err
	}
	var response WsResponse
...
func (e *Exchange) processOrderbook(respRaw []byte, channels []string) error {
	...
	if len(channels) == 3 {
		a, cp, err := e.getAssetPairByInstrument(orderbookData.InstrumentName)
		if err != nil {
			return err
		}
