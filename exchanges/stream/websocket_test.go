...
func TestAddWebsocket(t *testing.T) {
	...
	websocketSetup.GenerateSubscriptions = func() ([]ChannelSubscription, error) { return []ChannelSubscription{}, nil }
	_, err = websocketWrapper.AddWebsocket(websocketSetup)
	...
	websocketSetup.Subscriber = func(cs []ChannelSubscription) error { return nil }
	_, err = websocketWrapper.AddWebsocket(websocketSetup)
	if !errors.Is(err, errWebsocketUnsubscriberUnset) {
		t.Fatalf("received: '%v' but expected: '%v'", err, errInvalidWebsocketURL)
	}

	websocketSetup.Unsubscriber = func(cs []ChannelSubscription) error { return nil }
	_, err = websocketWrapper.AddWebsocket(websocketSetup)
