...
var defaultWrapperSetup = &WebsocketSetup{
	DefaultURL:   "testDefaultURL",
	RunningURL:   "wss://testRunningURL",
	Connector:    func() error { return nil },
	Subscriber:   func(_ []ChannelSubscription) error { return nil },
...
// 	t.Parallel()
// 	var wr *WrapperWebsocket
// 	_, err := wr.AddWebsocket(nil)
// 	if !errors.Is(err, errWebsocketIsNil) {
// 		t.Fatalf("received: '%v' but expected: '%v'", err, errWebsocketIsNil)
// 	}
// 	wr.AssetTypeWebsockets = make(map[asset.Item]*Websocket)
// 	*wr.AssetTypeWebsockets[asset.Spot] = &Websocket{DataHandler: make(chan interface{})}
// 	err = wr.AddWebsocket(nil)
...
// 	t.Parallel()
// 	wr := NewWrapper()
// wr.AssetTypeWebsockets[]
// wsInit := Websocket{}
// err := wr.AssetTypeWebsockets[asset.Spot].(&WebsocketSetup{
