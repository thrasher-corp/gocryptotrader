...
func (w *WebsocketConnection) SendJSONMessage(data interface{}) error {
	...
	if w.Verbose {
		log.Debugf(log.WebsocketMgr, "%s websocket connection: sending message to websocket %+v", w.ExchangeName, data)
	}

	if w.RateLimit > 0 {
		// TODO: Add context cancellation to avoid blocking and/or use request package limiter
		time.Sleep(time.Duration(w.RateLimit) * time.Millisecond)
	}
	// Check if the connection is still active after write control lock is
	// acquired.
	if !w.connected.Load() {
...
func (w *WebsocketConnection) SendRawMessage(messageType int, message []byte) error {
	...
	if w.Verbose {
		log.Debugf(log.WebsocketMgr, "%v websocket connection: sending message [%s]", w.ExchangeName, message)
	}
	if w.RateLimit > 0 {
		// TODO: Add context cancellation to avoid blocking and/or use request package limiter
		time.Sleep(time.Duration(w.RateLimit) * time.Millisecond)
...
func (w *WebsocketConnection) ReadMessage() Response { // TODO: Return error
	mType, resp, err := w.Connection.ReadMessage()
	if err != nil {
		if IsDisconnectionError(err) && w.connected.Swap(false) {
			// NOTE: When w.connected.Swap(false) returns true the underlying
...
func (w *WebsocketConnection) GetType() string {
	return w.Type
}
