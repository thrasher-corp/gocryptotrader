...
func createSnapshot() (holder *Orderbook, asks, bids orderbook.Items, err error) {
	...
	newBook := make(map[key.ExchangePairAssetKey]*orderbookHolder)
...
func BenchmarkUpdateBidsByPrice(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		holder := ob.ob[key.ExchangePairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}]
...
func TestUpdates(t *testing.T) {
	...
	book := holder.ob[key.ExchangePairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}]
...
func TestRunSnapshotWithNoData(t *testing.T) {
	t.Parallel()
	var obl Orderbook
	obl.ob = make(map[key.ExchangePairAssetKey]*orderbookHolder)
...
func TestFlushBuffer(t *testing.T) {
	...
	if obl.ob[key.ExchangePairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}] == nil {
...
func TestInsertingSnapShots(t *testing.T) {
	...
	holder.ob = make(map[key.ExchangePairAssetKey]*orderbookHolder)
	var snapShot1 orderbook.Base
...
func TestGetOrderbook(t *testing.T) {
	...
	bufferOb := holder.ob[key.ExchangePairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}]
