...
func createSnapshot() (holder *Orderbook, asks, bids orderbook.Items, err error) {
	...
	newBook := make(map[key.PairAssetKey]*orderbookHolder)
...
func BenchmarkUpdateBidsByPrice(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		holder := ob.ob[key.PairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}]
...
func TestUpdates(t *testing.T) {
	...
	book := holder.ob[key.PairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}]
...
func TestRunSnapshotWithNoData(t *testing.T) {
	t.Parallel()
	var obl Orderbook
	obl.ob = make(map[key.PairAssetKey]*orderbookHolder)
...
func TestFlushBuffer(t *testing.T) {
	...
	if obl.ob[key.PairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}] == nil {
...
func TestInsertingSnapShots(t *testing.T) {
	...
	holder.ob = make(map[key.PairAssetKey]*orderbookHolder)
	var snapShot1 orderbook.Base
...
func TestGetOrderbook(t *testing.T) {
	...
	bufferOb := holder.ob[key.PairAssetKey{Base: cp.Base.Item, Quote: cp.Quote.Item, Asset: asset.Spot}]
