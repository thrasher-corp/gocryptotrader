...
func (w *Orderbook) Setup(exchangeConfig *config.Exchange, c *Config, dataHandler chan<- interface{}) error {
	...
	w.ob = make(map[key.ExchangePairAssetKey]*orderbookHolder)
...
func (w *Orderbook) Update(u *orderbook.Update) error {
	...
	w.mtx.Lock()
	defer w.mtx.Unlock()
	book, ok := w.ob[key.ExchangePairAssetKey{Base: u.Pair.Base.Item, Quote: u.Pair.Quote.Item, Asset: u.Asset}]
	if !ok {
...
func (w *Orderbook) LoadSnapshot(book *orderbook.Base) error {
	...
	w.mtx.Lock()
	defer w.mtx.Unlock()
	holder, ok := w.ob[key.ExchangePairAssetKey{Base: book.Pair.Base.Item, Quote: book.Pair.Quote.Item, Asset: book.Asset}]
	if !ok {
		...
		w.ob[key.ExchangePairAssetKey{Base: book.Pair.Base.Item, Quote: book.Pair.Quote.Item, Asset: book.Asset}] = holder
	}
...
func (w *Orderbook) GetOrderbook(p currency.Pair, a asset.Item) (*orderbook.Base, error) {
	w.mtx.Lock()
	defer w.mtx.Unlock()
	book, ok := w.ob[key.ExchangePairAssetKey{Base: p.Base.Item, Quote: p.Quote.Item, Asset: a}]
	if !ok {
