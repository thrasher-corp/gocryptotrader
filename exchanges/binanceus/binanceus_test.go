...
func TestServerStatus(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipUnsetCredentials(t, bi)
...
func TestSubmitOrder(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipCredentialsSetCantManipulate(t, bi, canManipulateRealOrders)
	...
	switch {
	case sharedtestvalues.AreAPIkeysSet(bi) && err != nil && strings.Contains(err.Error(), "{\"code\":-1013,\"msg\":\"Market is closed.\""):
		t.Skip("Binanceus SubmitOrder() Market is Closed")
	case sharedtestvalues.AreAPIkeysSet(bi) && err != nil:
		t.Errorf("Binanceus SubmitOrder() Could not place order: %v", err)
	case sharedtestvalues.AreAPIkeysSet(bi) && response.Status != order.Filled:
...
func TestCancelOrder(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipUnsetCredentials(t, bi, canManipulateRealOrders)
...
func TestWithdraw(t *testing.T) {
	...
	} else if !sharedtestvalues.AreAPIkeysSet(bi) && err == nil {
