...
func (bi *Binanceus) ProcessUpdate(cp currency.Pair, a asset.Item, ws *WebsocketDepthStream) error {
	...
	for i := range ws.UpdateBids {
		...
		updateBid[i] = orderbook.Level{Price: p, Amount: a}
	}
	...
	for i := range ws.UpdateAsks {
		...
		updateAsk[i] = orderbook.Level{Price: p, Amount: a}
	}
...
func (o *orderbookManager) checkAndProcessUpdate(processor func(currency.Pair, asset.Item, *WebsocketDepthStream) error, pair currency.Pair, recent *orderbook.Book) error {
	o.Lock()
	defer o.Unlock()
	state, ok := o.state[pair.Base][pair.Quote][asset.Spot]
	if !ok {
