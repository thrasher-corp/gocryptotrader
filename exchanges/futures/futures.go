...
func (c *PositionController) TrackNewOrder(d *order.Detail) error {
	...
	c.m.Lock()
	defer c.m.Unlock()
	exchMap, ok := c.multiPositionTrackers[key.NewExchangePairAssetKey(d.Exchange, d.AssetType, d.Pair)]
	if !ok {
		...
		c.multiPositionTrackers[key.NewExchangePairAssetKey(d.Exchange, d.AssetType, d.Pair)] = exchMap
	}
...
func (c *PositionController) SetCollateralCurrency(exch string, item asset.Item, pair currency.Pair, collateralCurrency currency.Code) error {
	...
	tracker := c.multiPositionTrackers[key.NewExchangePairAssetKey(exch, item, pair)]
	if tracker == nil {
...
func (c *PositionController) TrackFundingDetails(d *fundingrate.HistoricalRates) error {
	...
	c.m.Lock()
	defer c.m.Unlock()
	tracker := c.multiPositionTrackers[key.NewExchangePairAssetKey(d.Exchange, d.Asset, d.Pair)]
	if tracker == nil {
...
func (c *PositionController) ClearPositionsForExchange(exch string, item asset.Item, pair currency.Pair) error {
	...
	c.multiPositionTrackers[key.NewExchangePairAssetKey(exch, item, pair)] = newMPT
	return nil
}
