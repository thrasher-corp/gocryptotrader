...
func (h *HitBTC) GetCurrencies() (map[string]Currencies, error) {
	...
	err := h.SendHTTPRequest(spotURL, path, &resp.Data)
	if err != nil {
		return ret, err
	}
...
func (h *HitBTC) GetCurrency(currency string) (Currencies, error) {
	...
	return resp.Data, h.SendHTTPRequest(spotURL, path, &resp.Data)
}
...
func (h *HitBTC) GetSymbols(symbol string) ([]string, error) {
	...
	err := h.SendHTTPRequest(spotURL, path, &resp)
	if err != nil {
		return ret, err
	}
...
func (h *HitBTC) GetSymbolsDetailed() ([]Symbol, error) {
	...
	return resp, h.SendHTTPRequest(spotURL, path, &resp)
}
