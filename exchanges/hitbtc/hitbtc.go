...
func (h *HitBTC) GetCurrencies() (map[string]Currencies, error) {
	...
	err := h.SendHTTPRequest(exchange.DefaultSpot, path, &resp.Data)
	if err != nil {
		return ret, err
	}
...
func (h *HitBTC) GetCurrency(currency string) (Currencies, error) {
	...
	return resp.Data, h.SendHTTPRequest(exchange.DefaultSpot, path, &resp.Data)
}
...
func (h *HitBTC) GetSymbols(symbol string) ([]string, error) {
	...
	err := h.SendHTTPRequest(exchange.DefaultSpot, path, &resp)
	if err != nil {
		return ret, err
	}
...
func (h *HitBTC) GetSymbolsDetailed() ([]Symbol, error) {
	...
	return resp, h.SendHTTPRequest(exchange.DefaultSpot, path, &resp)
}
...
func (h *HitBTC) SendHTTPRequest(ep, path string, result interface{}) error {
	endpoint, err := h.API.Endpoints.Get(ep)
	if err != nil {
		return err
	}
