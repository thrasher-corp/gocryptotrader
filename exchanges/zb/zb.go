...
func (z *ZB) SpotNewOrder(arg SpotNewOrderRequestParams) (int64, error) {
	...
	err := z.SendAuthenticatedHTTPRequest(secondaryRest, http.MethodGet, vals, &result, request.Auth)
	if err != nil {
		return 0, err
	}
	if result.Code != 1000 {
...
func (z *ZB) GetAccountInformation() (AccountsResponse, error) {
	...
	return result, z.SendAuthenticatedHTTPRequest(secondaryRest, http.MethodGet, vals, &result, request.Auth)
}
...
func (z *ZB) GetOrders(currency string, pageindex, side int64) ([]Order, error) {
	...
	return response, z.SendAuthenticatedHTTPRequest(secondaryRest, http.MethodGet, vals, &response, request.Auth)
}
...
func (z *ZB) GetMarkets() (map[string]MarketResponseItem, error) {
	...
	err := z.SendHTTPRequest(exchange.DefaultRest, endpoint, &res, request.UnAuth)
	if err != nil {
		return nil, err
	}
...
func (z *ZB) GetTicker(symbol string) (TickerResponse, error) {
	...
	err := z.SendHTTPRequest(exchange.DefaultRest, urlPath, &res, request.UnAuth)
	return res, err
}
...
func (z *ZB) GetTickers() (map[string]TickerChildResponse, error) {
	...
	err := z.SendHTTPRequest(exchange.DefaultRest, urlPath, &resp, request.UnAuth)
	return resp, err
}
...
func (z *ZB) GetSpotKline(arg KlinesRequestParams) (KLineResponse, error) {
	...
	err := z.SendHTTPRequest(exchange.DefaultRest, urlPath, &rawKlines, klineFunc)
	if err != nil {
		return res, err
	}
...
func (z *ZB) GetCryptoAddress(currency currency.Code) (UserAddress, error) {
	...
	return resp,
		z.SendAuthenticatedHTTPRequest(secondaryRest, http.MethodGet, vals, &resp, request.Auth)
}
...
func (z *ZB) SendAuthenticatedHTTPRequest(ep, httpMethod string, params url.Values, result interface{}, f request.EndpointLimit) error {
...
func (z *ZB) Withdraw(currency, address, safepassword string, amount, fees float64, itransfer bool) (string, error) {
	...
	var resp response
	err := z.SendAuthenticatedHTTPRequest(secondaryRest, http.MethodGet, vals, &resp, request.Auth)
	if err != nil {
		return "", err
	}
	if resp.Code != 1000 {
