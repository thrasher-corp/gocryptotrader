...
func GetHoldings(exch string, creds *Credentials, assetType asset.Item) (Holdings, error) {
	...
	for mapKey, assetHoldings := range subAccountHoldings {
		...
		for currItem, bal := range assetHoldings.balances {
			bal.m.Lock()
...
// Load loads the holdings with new account info
func (s *Service) Load(incoming *Holdings, creds *Credentials) error {
	if incoming == nil {
	...
	if !exist {
		...
		accounts = &Accounts{
			ID:          id,
			subAccounts: make(map[Credentials]map[key.SubAccountAsset]subAccountAsset),
		}
	...
	if !exist {
		subAccounts = make(map[key.SubAccountAsset]subAccountAsset)
	...
	for x := range incoming.Accounts {
		...
		if !exist {
			assets = subAccountAsset{
				balances: make(map[*currency.Item]*ProtectedBalance),
			}
			accounts.subAccounts[*creds][accAsset] = assets
		}

		updated := make(map[*currency.Item]any)
		for y := range incoming.Accounts[x].Currencies {
			...
			bal, ok := assets.balances[accBal.Currency.Item]
			if !ok || bal == nil {
			...
			assets.balances[accBal.Currency.Item] = bal
			updated[accBal.Currency.Item] = new(any)
		}
		for cur, bal := range assets.balances {
			if _, ok := updated[cur]; !ok {
				bal.clean()
			}
		}
