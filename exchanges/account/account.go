...
func SubscribeToExchangeAccount(exchange string) (dispatch.Pipe, error) {
	...
	if !ok {
		var err error
		if accounts, err = service.trackNewAccounts(exchange); err != nil {
...
func (s *Service) Save(incoming *Holdings, creds *Credentials) error {
	...
	if !ok {
		var err error
		if accounts, err = s.trackNewAccounts(exch); err != nil {
...
func (s *Service) Update(exch string, changes []Change, creds *Credentials) error {
	if exch == "" {
		return fmt.Errorf("cannot update balance: %w", errExchangeNameUnset)
	}

	if creds.IsEmpty() {
		return fmt.Errorf("cannot update balance: %w", errCredentialsAreNil)
	}
	...
	if !ok {
		var err error
		if accounts, err = s.trackNewAccounts(exch); err != nil {
			return fmt.Errorf("cannot update balance for %s %w", exch, err)
		}
	}

	var subAccounts map[key.SubAccountAsset]currencyBalances
	subAccounts, ok = accounts.subAccounts[*creds]
	if !ok {
		subAccounts = make(map[key.SubAccountAsset]currencyBalances)
		accounts.subAccounts[*creds] = subAccounts
	...
	for _, change := range changes {
		if !change.AssetType.IsValid() {
			errs = common.AppendError(errs, fmt.Errorf("cannot update balance for %s [%s] %w",
				change.Account, change.AssetType, asset.ErrNotSupported))
			continue
		}
		if change.Balance == nil {
			errs = common.AppendError(errs, fmt.Errorf("cannot update balance for %s [%s] %w",
				change.Account, change.AssetType, errBalanceIsNil))
			continue
		}
		...
		if err := bal.load(change.Balance); err != nil {
			errs = common.AppendError(errs, fmt.Errorf("cannot update balance for %s [%s %s] %w",
				change.Account,
				change.AssetType,
