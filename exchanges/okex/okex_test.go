...
func TestOrderBookUpdateChecksumCalculator(t *testing.T) {
	...
	_ = o.WsProcessOrderBook(dataResponse, false)
	var updateResponse okgroup.WebsocketDataResponse
	err = common.JSONDecode([]byte(update), &updateResponse)
	if err != nil {
		t.Error(err)
	}
	orderbookUpdate := o.WsProcessOrderBook(updateResponse, true)
	for i := range dataResponse.Data {
		if dataResponse.Data[i].Checksum != o.ChecksumPartialCalculationAlternatingViaWSMessage(dataResponse.Data[i]) {
			t.Error("Checksum for partial does not match")
		}
		if updateResponse.Data[i].Checksum != o.ChecksumUpdateCalculationAlternatingViaOrderBook(orderbookUpdate) {
			t.Error("Checksum for partial does not match")
		}
	}
}
...
func TestOrderBookPartialChecksumCalculator(t *testing.T) {
	...
	calculatedChecksum := o.ChecksumPartialCalculationAlternatingViaWSMessage(dataResponse.Data[0])
