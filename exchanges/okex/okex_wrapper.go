...
const okexWithdrawalPermissions = exchange.AutoWithdrawCrypto |
	exchange.NoFiatWithdrawals
const okexKlineResultLimit = 1440
...
func (o *OKEX) SetDefaults() {
	...
	err = o.Protocol.SetGlobals(&protocol.Globals{
		WithdrawalPermissions: okexWithdrawalPermissions,
...
func (o *OKEX) GetHistoricCandlesExtended(pair currency.Pair, a asset.Item, start, end time.Time, interval kline.Interval) (kline.Item, error) {
	...
	dates := kline.CalcDateRanges(start, end, interval, okexKlineResultLimit)
	formattedPair, err := o.FormatExchangeCurrency(pair, a)
	if err != nil {
		return kline.Item{}, err
	}
