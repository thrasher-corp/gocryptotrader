...
// initial operations
func (p *Poloniex) GetPlatformHistory(cp currency.Pair, assetType assets.AssetType, timestampStart time.Time, tradeID string) ([]exchange.PlatformTrade, error) {
	...
	t, err := p.GetTradeHistory(cp.String(),
	...
	for i := range t {
		cTime, err := ConvertTimeStringToRFC3339(t[i].Date)
		if err != nil {
			return resp, err
		}
