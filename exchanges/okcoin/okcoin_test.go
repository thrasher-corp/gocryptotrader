...
func TestGetAccountCurrencies(t *testing.T) {
	...
	if !sharedtestvalues.AreAPIkeysSet(o) && err == nil {
...
func TestGetAccountWalletInformation(t *testing.T) {
	...
	if sharedtestvalues.AreAPIkeysSet(o) {
		if err != nil {
			t.Error(err)
		}
		if len(resp) == 0 {
	...
	} else if !sharedtestvalues.AreAPIkeysSet(o) && err == nil {
		t.Error("Expecting an error when no keys are set")
	}
}
...
func TestTransferAccountFunds(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipUnsetCredentials(t, o, canManipulateRealOrders)
...
func TestPlaceSpotOrderLimit(t *testing.T) {
	t.Parallel()
	sharedtestvalues.SkipCredentialsSetCantManipulate(t, o, canManipulateRealOrders)
...
func TestSubmitOrder(t *testing.T) {
	...
	if sharedtestvalues.AreAPIkeysSet(o) && (err != nil || response.Status != order.New) {
