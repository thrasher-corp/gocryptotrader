...
func (o *Okcoin) manageSubscriptions(operation string, subs subscription.List) error {
	...
	for i := 0; i < len(subs); i++ {
		...
		if len(chunk) > maxConnByteLen {
			...
			if operation == "unsubscribe" {
				if authenticatedChannelSubscription {
					err = o.Websocket.RemoveSubscriptions(nil, authChannels...)
				} else {
					err = o.Websocket.RemoveSubscriptions(nil, channels...)
				}
			} else {
				if authenticatedChannelSubscription {
	...
	if operation == "unsubscribe" {
		return o.Websocket.RemoveSubscriptions(nil, channels...)
	}
	return o.Websocket.AddSuccessfulSubscriptions(nil, channels...)
}
