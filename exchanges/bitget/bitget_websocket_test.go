...
func (c errConnection) Dial(context.Context, *gws.Dialer, http.Header, url.Values) error {
...
func (c pingConnection) Dial(context.Context, *gws.Dialer, http.Header, url.Values) error {
	return nil
}

func (c pingConnection) SetupPingHandler(request.EndpointLimit, websocket.PingHandler) {}
...
func TestWsHandleData(t *testing.T) {
	...
	tc := []jsonExpectedErr{
		{j: []byte(`pong`), err: nil},
		{j: []byte(`{"event":"subscribe"}`), err: nil},
		{j: []byte(`{"event":"login","code":0}`), err: nil},
		{j: []byte(`{"event":"login","code":1}`), err: nil},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"fakeChannelNotReal"}}`), err: nil},
		{j: []byte(`{"event":"update","arg":{"channel":"fakeChannelNotReal"}}`), err: nil},
		{j: []byte(`{"event":"fakeEventNotReal"}`), err: nil},
	}
	for i := range tc {
...
func TestTickerDataHandler(t *testing.T) {
	t.Parallel()
	tc := []jsonExpectedErr{
		{j: []byte(`{"event":"snapshot","arg":{"channel":"ticker","instType":"SPOT"},"data":[{}]}`), err: errUnknownPairQuote},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"ticker","instType":"SPOT"},"data":[{"InstId":"BTCUSD"}]}`), err: nil},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"ticker","instType":"USDT-FUTURES"},"data":[{}]}`), err: errUnknownPairQuote},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"ticker","instType":"USDT-FUTURES"},"data":[{"InstId":"BTCUSD"}]}`), err: nil},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"ticker","instType":"moo"},"data":[{"InstId":"BTCUSD"}]}`), err: nil},
	}
	for i := range tc {
		err := e.wsHandleData(t.Context(), fixtureConnection{}, tc[i].j)
		assert.ErrorIs(t, err, tc[i].err, "test %d failed", i)
...
func TestCandleDataHandler(t *testing.T) {
	t.Parallel()
	tc := []jsonExpectedErr{
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D"},"data":[["1","2","3","4","5","6","",""]]}`), err: errUnknownPairQuote},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["a","2","3","4","5","6","",""]]}`), err: strconv.ErrSyntax},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["1","a","3","4","5","6","",""]]}`), err: strconv.ErrSyntax},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["1","2","a","4","5","6","",""]]}`), err: strconv.ErrSyntax},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["1","2","3","a","5","6","",""]]}`), err: strconv.ErrSyntax},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["1","2","3","4","a","6","",""]]}`), err: strconv.ErrSyntax},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["1","2","3","4","5","a","",""]]}`), err: strconv.ErrSyntax},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"candle1D","instId":"BTCUSD"},"data":[["1","2","3","4","5","6","",""]]}`), err: nil},
	}
	for i := range tc {
		err := e.wsHandleData(t.Context(), fixtureConnection{}, tc[i].j)
		assert.ErrorIs(t, err, tc[i].err, "test %d failed", i)
...
func TestAccountSnapshotDataHandler(t *testing.T) {
	...
	tc := []jsonExpectedErr{
		{j: []byte(`{"event":"snapshot","arg":{"channel":"account"},"data":[]}`), err: asset.ErrNotSupported},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"account","instType":"spot"},"data":[{}]}`), err: nil},
		{j: []byte(`{"event":"snapshot","arg":{"channel":"account","instType":"USDT-FUTURES"},"data":[{}]}`), err: nil},
	}
	for i := range tc {
