...
func TestSubscribeSavings(t *testing.T) {
	...
	_, err = e.SubscribeSavings(t.Context(), resp[0].ProductID, resp[0].PeriodType, resp2.SingleMinAmount)
	assert.NoError(t, err)
}
...
func TestSubscribeSharkFin(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotEmpty(t, resp)
	_, err = e.SubscribeSharkFin(t.Context(), resp.ResultList[0].ProductID, resp.ResultList[0].MinimumAmount)
	assert.NoError(t, err)
}
...
func TestGetOrderInfo(t *testing.T) {
	...
	require.NoError(t, err)
	if len(resp) != 0 {
		_, err = e.GetOrderInfo(t.Context(), strconv.FormatInt(int64(resp[0].OrderID), 10), testPair, asset.Spot)
		assert.NoError(t, err)
	}
}
...
func TestCommitConversion(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = e.CommitConversion(t.Context(), testCrypto, testFiat, resp.TraceID, resp.FromCoinSize, resp.ToCoinSize, resp.ConvertPrice)
	assert.NoError(t, err)
}
