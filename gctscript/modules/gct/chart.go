...
var chartsModule = map[string]objects.Object {
	"chart": &objects.UserFunction{Name: "chart", Value: generateChart},
}
...
func generateChart(args ...objects.Object) (objects.Object, error) {
	...
	chartType, ok := objects.ToString(args[0])
	if !ok {
		return nil, fmt.Errorf(ErrParameterConvertFailed, chartType)
	}
	...
	if !valid {
		return nil, fmt.Errorf(modules.ErrParameterConvertFailed, chartType)
	}
	...
	chart := charts.New("helloworlds","timeseries", OutputDir)
	var err error
