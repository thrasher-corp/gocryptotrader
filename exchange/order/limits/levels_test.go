...
func TestConforms(t *testing.T) {
	...
	err = tt.Conforms(200, .5, order.Limit)
	assert.NoError(t, err)

	tt.PriceStepIncrementSize = 0.001
	err = tt.Conforms(200.0001, .5, order.Limit)
	assert.ErrorIs(t, err, ErrPriceExceedsStep)

	err = tt.Conforms(200.004, .5, order.Limit)
	assert.NoError(t, err)

	tt.AmountStepIncrementSize = 0.001
	err = tt.Conforms(200, .0002, order.Limit)
	assert.ErrorIs(t, err, ErrAmountExceedsStep)
	err = tt.Conforms(200000, .003, order.Limit)
	assert.NoError(t, err)
...
func TestConformToPrice(t *testing.T) {
	t.Parallel()
	tt := &MinMaxLevel{}
	resp := tt.ConformToPrice(1.0)
	assert.Equal(t, 1.0, resp)

	tt.PriceStepIncrementSize = 1

	resp = tt.ConformToPrice(1.5)
	assert.Equal(t, 1.0, resp)

	resp = tt.ConformToPrice(0.5)
