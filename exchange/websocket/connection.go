...
type Connection interface {
	...
	SendMessageReturnResponseWithRateLimitWeight(ctx context.Context, epl request.EndpointLimit, weight request.Weight, signature, request any) ([]byte, error)
...
func (c *connection) SendJSONMessage(ctx context.Context, epl request.EndpointLimit, data any) error {
	return c.writeToConn(ctx, epl, 0, func() error {
...
func (c *connection) SendRawMessage(ctx context.Context, epl request.EndpointLimit, messageType int, message []byte) error {
	return c.sendRawMessage(ctx, epl, 0, messageType, message)
}

func (c *connection) sendRawMessage(ctx context.Context, epl request.EndpointLimit, weight request.Weight, messageType int, message []byte) error {
...
func (c *connection) writeToConn(ctx context.Context, epl request.EndpointLimit, weight request.Weight, writeConn func() error) error {
	if !c.IsConnected() {
	...
	if rl != nil {
		var err error
		if weight > 0 {
			err = rl.RateLimitWithWeight(ctx, weight)
		} else {
...
func (c *connection) SendMessageReturnResponse(ctx context.Context, epl request.EndpointLimit, signature, payload any) ([]byte, error) {
	resps, err := c.sendMessageReturnResponses(ctx, epl, 0, signature, payload, 1, nil)
	if err != nil {
		return nil, err
	}
	return resps[0], nil
...
// SendMessageReturnResponseWithRateLimitWeight will send a WS message using a request-specific endpoint rate-limit weight.
func (c *connection) SendMessageReturnResponseWithRateLimitWeight(ctx context.Context, epl request.EndpointLimit, weight request.Weight, signature, payload any) ([]byte, error) {
	resps, err := c.sendMessageReturnResponses(ctx, epl, weight, signature, payload, 1, nil)
	if err != nil {
		return nil, err
	}
	return resps[0], nil
...
func (c *connection) SendMessageReturnResponses(ctx context.Context, epl request.EndpointLimit, signature, payload any, expected int) ([][]byte, error) {
	return c.sendMessageReturnResponses(ctx, epl, 0, signature, payload, expected, nil)
}
...
func (c *connection) sendMessageReturnResponses(ctx context.Context, epl request.EndpointLimit, weight request.Weight, signature, payload any, expected int, messageInspector Inspector) ([][]byte, error) {
	...
	start := time.Now()
	if err := c.sendRawMessage(ctx, epl, weight, gws.TextMessage, outbound); err != nil {
		return nil, err
	}
