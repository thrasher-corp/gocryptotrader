...
func TestNumConnections(t *testing.T) {
	...
	websocketConnFilters := []string{"spotPublic", "spotTrade", "spotAuth", "optionsPublic", "optionsAuthenticated"}
	for _, mesageFilter := range websocketConnFilters {
		err = multi.SetupNewConnection(&ConnectionSetup{
			ResponseCheckTimeout:  time.Millisecond,
			URL:                   "urlstring",
