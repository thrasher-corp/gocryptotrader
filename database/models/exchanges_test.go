...
func testExchangeToManyExchangeTradeHistories(t *testing.T) {
	var err error
	tx := MustTx(boil.Begin())
	defer tx.Rollback()

	var a Exchange
	var b, c ExchangeTradeHistory
	...
	randomize.Struct(seed, &b, exchangeTradeHistoryDBTypes, false, exchangeTradeHistoryColumnsWithDefault...)
	randomize.Struct(seed, &c, exchangeTradeHistoryDBTypes, false, exchangeTradeHistoryColumnsWithDefault...)
	...
	exchangeTradeHistory, err := a.ExchangeTradeHistories(tx).All()
	if err != nil {
		t.Fatal(err)
	}

	bFound, cFound := false, false
	for _, v := range exchangeTradeHistory {
		if v.ExchangeID == b.ExchangeID {
			bFound = true
		}
		if v.ExchangeID == c.ExchangeID {
	...
	slice := ExchangeSlice{&a}
	if err = a.L.LoadExchangeTradeHistories(tx, false, (*[]*Exchange)(&slice)); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ExchangeTradeHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ExchangeTradeHistories = nil
	if err = a.L.LoadExchangeTradeHistories(tx, true, &a); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ExchangeTradeHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testExchangeToManyAddOpExchangeTradeHistories(t *testing.T) {
	var err error
	...
	var a Exchange
	var b, c, d, e ExchangeTradeHistory

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, exchangeDBTypes, false, strmangle.SetComplement(exchangePrimaryKeyColumns, exchangeColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*ExchangeTradeHistory{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, exchangeTradeHistoryDBTypes, false, strmangle.SetComplement(exchangeTradeHistoryPrimaryKeyColumns, exchangeTradeHistoryColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*ExchangeTradeHistory{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddExchangeTradeHistories(tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ExchangeTradeHistories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ExchangeTradeHistories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ExchangeTradeHistories(tx).Count()
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
