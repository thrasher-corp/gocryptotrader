...
func testTradeToOneScriptUsingExchange(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadExchange(ctx, tx, false, (*[]*Trade)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTradeToOneSetOpScriptUsingExchange(t *testing.T) {
	var err error
...
func testTradeToOneRemoveOpScriptUsingExchange(t *testing.T) {
	var err error
...
var (
	tradeDBTypes = map[string]string{`ID`: `uuid`, `ExchangeID`: `uuid`, `Currency`: `character varying`, `Asset`: `character varying`, `Event`: `character varying`, `Price`: `timestamp without time zone`, `Amount`: `timestamp without time zone`, `Side`: `character varying`}
	_            = bytes.MinRead
)
