...
func (o *ScriptExecution) Script(mods ...qm.QueryMod) scriptEventQuery {
	...
	query := ScriptEvents(queryMods...)
	queries.SetFrom(query.Query, "\"script_event\"")
...
func (scriptExecutionL) LoadScript(ctx context.Context, e boil.ContextExecutor, singular bool, maybeScriptExecution interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`script_event`), qm.WhereIn(`script_event.id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load ScriptEvent")
	}

	var resultSlice []*ScriptEvent
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice ScriptEvent")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for script_event")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for script_event")
	}
	...
	if singular {
		...
		if foreign.R == nil {
			foreign.R = &scriptEventR{}
		}
		foreign.R.ScriptScriptExecutions = append(foreign.R.ScriptScriptExecutions, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.ScriptID, foreign.ID) {
				...
				foreign.R.ScriptScriptExecutions = append(foreign.R.ScriptScriptExecutions, local)
				break
			}
		}
	}
...
// Adds o to related.R.ScriptScriptExecutions.
func (o *ScriptExecution) SetScript(ctx context.Context, exec boil.ContextExecutor, insert bool, related *ScriptEvent) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &scriptEventR{
			ScriptScriptExecutions: ScriptExecutionSlice{o},
		}
	} else {
		related.R.ScriptScriptExecutions = append(related.R.ScriptScriptExecutions, o)
	}
...
func (o *ScriptExecution) RemoveScript(ctx context.Context, exec boil.ContextExecutor, related *ScriptEvent) error {
	var err error
	...
	for i, ri := range related.R.ScriptScriptExecutions {
		if queries.Equal(o.ScriptID, ri.ScriptID) {
			continue
		}

		ln := len(related.R.ScriptScriptExecutions)
		if ln > 1 && i < ln-1 {
			related.R.ScriptScriptExecutions[i] = related.R.ScriptScriptExecutions[ln-1]
		}
		related.R.ScriptScriptExecutions = related.R.ScriptScriptExecutions[:ln-1]
		break
	}
	return nil
}
