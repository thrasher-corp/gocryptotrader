...
var (
	orderEventDBTypes = map[string]string{`ID`: `bigint`, `ExchangeOrderID`: `character varying`, `ClientID`: `uuid`, `Exchange`: `character varying`, `CurrencyPair`: `character varying`, `AssetType`: `character varying`, `OrderType`: `character varying`, `OrderSide`: `character varying`, `OrderStatus`: `character varying`, `Amount`: `double precision`, `Price`: `double precision`, `UpdatedAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`}
	_                 = bytes.MinRead
)
...
func testOrderEventsUpsert(t *testing.T) {
	t.Parallel()
	...
	if err = o.Upsert(ctx, tx, false, nil, boil.Infer(), boil.Infer()); err != nil {
		t.Errorf("Unable to upsert OrderEvent: %s", err)
	}
	...
	if err = randomize.Struct(seed, &o, orderEventDBTypes, false, orderEventPrimaryKeyColumns...); err != nil {
		t.Errorf("Unable to randomize OrderEvent struct: %s", err)
	}
