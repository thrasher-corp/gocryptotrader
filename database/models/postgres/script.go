...
// ExchangeTrades retrieves all the trade's Trades with an executor via exchange_id column.
func (o *Script) ExchangeTrades(mods ...qm.QueryMod) tradeQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"trade\".\"exchange_id\"=?", o.ID),
	)
...
// LoadExchangeTrades allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (scriptL) LoadExchangeTrades(ctx context.Context, e boil.ContextExecutor, singular bool, maybeScript interface{}, mods queries.Applicator) error {
	var slice []*Script
	var object *Script
	...
	query := NewQuery(qm.From(`trade`), qm.WhereIn(`trade.exchange_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ExchangeTrades = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &tradeR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.ExchangeID) {
				local.R.ExchangeTrades = append(local.R.ExchangeTrades, foreign)
				if foreign.R == nil {
					foreign.R = &tradeR{}
				}
...
// AddExchangeTrades adds the given related objects to the existing relationships
// of the script, optionally inserting them as new records.
// Appends related to o.R.ExchangeTrades.
// Sets related.R.Exchange appropriately.
func (o *Script) AddExchangeTrades(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Trade) error {
	var err error
	for _, rel := range related {
		if insert {
	...
	if o.R == nil {
		o.R = &scriptR{
			ExchangeTrades: related,
		}
	} else {
		o.R.ExchangeTrades = append(o.R.ExchangeTrades, related...)
	}
...
// SetExchangeTrades removes all previously related items of the
// script replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Exchange's ExchangeTrades accordingly.
// Replaces o.R.ExchangeTrades with related.
// Sets related.R.Exchange's ExchangeTrades accordingly.
func (o *Script) SetExchangeTrades(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Trade) error {
	query := "update \"trade\" set \"exchange_id\" = null where \"exchange_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	if o.R != nil {
		for _, rel := range o.R.ExchangeTrades {
		...
		o.R.ExchangeTrades = nil
	}
	return o.AddExchangeTrades(ctx, exec, insert, related...)
}

// RemoveExchangeTrades relationships from objects passed in.
// Removes related items from R.ExchangeTrades (uses pointer comparison, removal does not keep order)
// Sets related.R.Exchange.
func (o *Script) RemoveExchangeTrades(ctx context.Context, exec boil.ContextExecutor, related ...*Trade) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ExchangeID, nil)
		if rel.R != nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.ExchangeTrades {
			if rel != ri {
				continue
			}

			ln := len(o.R.ExchangeTrades)
			if ln > 1 && i < ln-1 {
				o.R.ExchangeTrades[i] = o.R.ExchangeTrades[ln-1]
			}
			o.R.ExchangeTrades = o.R.ExchangeTrades[:ln-1]
			break
		}
	}
