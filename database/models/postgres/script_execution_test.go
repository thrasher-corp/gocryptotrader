...
func testScriptExecutionToOneScriptEventUsingScript(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local ScriptExecution
	var foreign ScriptEvent

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, scriptExecutionDBTypes, true, scriptExecutionColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize ScriptExecution struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, scriptEventDBTypes, false, scriptEventColumnsWithDefault...); err != nil {
...
func testScriptExecutionToOneSetOpScriptEventUsingScript(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, scriptEventDBTypes, false, strmangle.SetComplement(scriptEventPrimaryKeyColumns, scriptEventColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, scriptEventDBTypes, false, strmangle.SetComplement(scriptEventPrimaryKeyColumns, scriptEventColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*ScriptEvent{&b, &c} {
		...
		if x.R.ScriptScriptExecutions[0] != &a {
...
func testScriptExecutionToOneRemoveOpScriptEventUsingScript(t *testing.T) {
	var err error
	...
	if len(b.R.ScriptScriptExecutions) != 0 {
