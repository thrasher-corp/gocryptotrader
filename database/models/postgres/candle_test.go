...
var (
	candleDBTypes = map[string]string{`ID`: `uuid`, `ExchangeID`: `uuid`, `Base`: `character varying`, `Quote`: `character varying`, `Interval`: `character varying`, `Date`: `timestamp with time zone`, `Open`: `numeric`, `High`: `numeric`, `Low`: `numeric`, `Close`: `numeric`, `Volume`: `numeric`}
	_             = bytes.MinRead
)
