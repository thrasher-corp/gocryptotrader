...
var (
	tradeEventDBTypes = map[string]string{`ID`: `bigint`, `TradeID`: `character varying`, `OrderID`: `bigint`, `Exchange`: `character varying`, `BaseCurrency`: `character varying`, `QuoteCurrency`: `character varying`, `Side`: `character varying`, `Volume`: `double precision`, `Price`: `double precision`, `Fee`: `double precision`, `Tax`: `double precision`, `ExecutedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`}
	_                 = bytes.MinRead
)
...
func testTradeEventsUpsert(t *testing.T) {
	t.Parallel()
	...
	if err = o.Upsert(ctx, tx, false, nil, boil.Infer(), boil.Infer()); err != nil {
		t.Errorf("Unable to upsert TradeEvent: %s", err)
	}
	...
	if err = randomize.Struct(seed, &o, tradeEventDBTypes, false, tradeEventPrimaryKeyColumns...); err != nil {
		t.Errorf("Unable to randomize TradeEvent struct: %s", err)
	}
