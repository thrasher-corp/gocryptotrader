...
func testCandleToOneDatahistoryjobUsingRelatedJob(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&local.RelatedJobID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.RelatedJob().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadRelatedJob(ctx, tx, false, (*[]*Candle)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.RelatedJob == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.RelatedJob = nil
	if err = local.L.LoadRelatedJob(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.RelatedJob == nil {
		t.Error("struct should have been eager loaded")
...
func testCandleToOneSetOpDatahistoryjobUsingRelatedJob(t *testing.T) {
	var err error
	...
	for i, x := range []*Datahistoryjob{&b, &c} {
		err = a.SetRelatedJob(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
		...
		if x.R.RelatedJobCandles[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.RelatedJobID, x.ID) {
			t.Error("foreign key was wrong value", a.RelatedJobID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.RelatedJobID))
		reflect.Indirect(reflect.ValueOf(&a.RelatedJobID)).Set(zero)
		...
		if !queries.Equal(a.RelatedJobID, x.ID) {
			t.Error("foreign key was wrong value", a.RelatedJobID, x.ID)
		}
	}
}

func testCandleToOneRemoveOpDatahistoryjobUsingRelatedJob(t *testing.T) {
	var err error
	...
	if err = a.SetRelatedJob(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveRelatedJob(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.RelatedJob().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.RelatedJob != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.RelatedJobID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.RelatedJobCandles) != 0 {
...
var (
	candleDBTypes = map[string]string{`ID`: `TEXT`, `ExchangeNameID`: `UUID`, `Base`: `TEXT`, `Quote`: `TEXT`, `Interval`: `TEXT`, `Timestamp`: `TIMESTAMP`, `Open`: `REAL`, `High`: `REAL`, `Low`: `REAL`, `Close`: `REAL`, `Volume`: `REAL`, `Asset`: `TEXT`, `RelatedJobID`: `TEXT`, `ValidationIssues`: `TEXT`}
	_             = bytes.MinRead
)
