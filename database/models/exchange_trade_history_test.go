...
func testExchangeTradeHistoryToOneExchangeUsingExchange(t *testing.T) {
	tx := MustTx(boil.Begin())
	defer tx.Rollback()

	var local ExchangeTradeHistory
	var foreign Exchange

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, exchangeTradeHistoryDBTypes, false, exchangeTradeHistoryColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize ExchangeTradeHistory struct: %s", err)
	}
	...
	slice := ExchangeTradeHistorySlice{&local}
	if err = local.L.LoadExchange(tx, false, (*[]*ExchangeTradeHistory)(&slice)); err != nil {
		t.Fatal(err)
	}
...
func testExchangeTradeHistoryToOneSetOpExchangeUsingExchange(t *testing.T) {
	var err error

	tx := MustTx(boil.Begin())
	defer tx.Rollback()

	var a ExchangeTradeHistory
	var b, c Exchange

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, exchangeTradeHistoryDBTypes, false, strmangle.SetComplement(exchangeTradeHistoryPrimaryKeyColumns, exchangeTradeHistoryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*Exchange{&b, &c} {
		...
		if x.R.ExchangeTradeHistories[0] != &a {
