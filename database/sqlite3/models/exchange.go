...
// ClientOrderHistory pointed to by the foreign key.
func (o *Exchange) ClientOrderHistory(mods ...qm.QueryMod) clientOrderHistoryQuery {
...
// LoadClientOrderHistory allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (exchangeL) LoadClientOrderHistory(ctx context.Context, e boil.ContextExecutor, singular bool, maybeExchange interface{}, mods queries.Applicator) error {
	var slice []*Exchange
	var object *Exchange
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load ClientOrderHistory")
	}

	var resultSlice []*ClientOrderHistory
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice ClientOrderHistory")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for client_order_history")
	}
	...
	if singular {
		...
		object.R.ClientOrderHistory = foreign
		if foreign.R == nil {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ID == foreign.ExchangeID {
				local.R.ClientOrderHistory = foreign
				if foreign.R == nil {
					foreign.R = &clientOrderHistoryR{}
				}
...
// SetClientOrderHistory of the exchange to the related item.
// Sets o.R.ClientOrderHistory to related.
// Adds o to related.R.Exchange.
func (o *Exchange) SetClientOrderHistory(ctx context.Context, exec boil.ContextExecutor, insert bool, related *ClientOrderHistory) error {
	var err error
	...
	if o.R == nil {
		o.R = &exchangeR{
			ClientOrderHistory: related,
		}
	} else {
		o.R.ClientOrderHistory = related
	}

	if related.R == nil {
		related.R = &clientOrderHistoryR{
			Exchange: o,
		}
	} else {
...
func (o *Exchange) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for exchange")
	}
