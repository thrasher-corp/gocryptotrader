...
var ExchangeRels = struct {
	ExchangePlatformTradeHistories string
	Keys                           string
	UserOrderHistories             string
}{
	ExchangePlatformTradeHistories: "ExchangePlatformTradeHistories",
	Keys:                           "Keys",
...
type exchangeR struct {
	ExchangePlatformTradeHistories ExchangePlatformTradeHistorySlice
	Keys                           KeySlice
...
var (
	...
	exchangeColumnsWithoutDefault = []string{"exchange_name", "created_at", "updated_at"}
...
// ExchangePlatformTradeHistories retrieves all the exchange_platform_trade_history's ExchangePlatformTradeHistories with an executor.
func (o *Exchange) ExchangePlatformTradeHistories(mods ...qm.QueryMod) exchangePlatformTradeHistoryQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"exchange_platform_trade_history\".\"exchange_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"exchange_platform_trade_history\".*"})
	}
...
func (o *Exchange) UserOrderHistories(mods ...qm.QueryMod) userOrderHistoryQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"user_order_history\".\"exchange_id\"=?", o.ID),
	)
...
// LoadExchangePlatformTradeHistories allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (exchangeL) LoadExchangePlatformTradeHistories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeExchange interface{}, mods queries.Applicator) error {
	var slice []*Exchange
	var object *Exchange
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load exchange_platform_trade_history")
	}

	var resultSlice []*ExchangePlatformTradeHistory
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice exchange_platform_trade_history")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on exchange_platform_trade_history")
	}
	...
	if singular {
		object.R.ExchangePlatformTradeHistories = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.ExchangeID {
				local.R.ExchangePlatformTradeHistories = append(local.R.ExchangePlatformTradeHistories, foreign)
				if foreign.R == nil {
					foreign.R = &exchangePlatformTradeHistoryR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (exchangeL) LoadUserOrderHistories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeExchange interface{}, mods queries.Applicator) error {
	var slice []*Exchange
	var object *Exchange
...
// AddExchangePlatformTradeHistories adds the given related objects to the existing relationships
// of the exchange, optionally inserting them as new records.
// Appends related to o.R.ExchangePlatformTradeHistories.
// Sets related.R.Exchange appropriately.
func (o *Exchange) AddExchangePlatformTradeHistories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ExchangePlatformTradeHistory) error {
	var err error
	...
	if o.R == nil {
		o.R = &exchangeR{
			ExchangePlatformTradeHistories: related,
		}
	} else {
		o.R.ExchangePlatformTradeHistories = append(o.R.ExchangePlatformTradeHistories, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &exchangePlatformTradeHistoryR{
				Exchange: o,
			}
		} else {
			rel.R.Exchange = o
...
// Sets related.R.Exchange appropriately.
func (o *Exchange) AddUserOrderHistories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserOrderHistory) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.ExchangeID = o.ID
...
func FindExchange(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Exchange, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"exchanges\" where \"id\"=$1", sel,
	)
...
func (o *Exchange) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"exchanges\" %sDEFAULT VALUES%s"
		}
...
func (o *Exchange) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"exchanges\"", updateOnConflict, ret, update, conflict, insert)
...
func ExchangeExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"exchanges\" where \"id\"=$1 limit 1)"
