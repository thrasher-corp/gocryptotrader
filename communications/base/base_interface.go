...
func (c IComm) Setup() error {
	...
	if count == 0 {
		log.Println("Warning no communications packages are enabled")
	}
	return nil
}
...
func (c IComm) GetEnabledCommunicationMediums() {
	...
	for i := range c {
		if c[i].IsEnabled() && c[i].IsConnected() {
			log.Printf("%s is enabled", c[i].GetName())
			count++
		}
	}
	if count == 0 {
		log.Println("No communications packages enabled or not connected, please enable in config.json")
	}
}
