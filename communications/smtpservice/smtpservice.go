...
func (s *SMTPservice) Setup(cfg *config.CommunicationsConfig) {
	...
	log.Debugf(log.SubSystemCommMgr, "SMTP: Setup - From: %v. To: %s. Server: %s.", s.From, s.RecipientList, s.Host)
}
...
func (s *SMTPservice) Send(subject, msg string) error {
	...
	log.Debugf(log.SubSystemCommMgr, "SMTP: Sending email to %v. Subject: %s Message: %s [From: %s]", s.RecipientList,
		subject, msg, s.From)
