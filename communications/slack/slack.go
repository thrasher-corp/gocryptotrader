...
func (s *Slack) NewConnection() error {
	if !s.Connected {
		...
		if s.Verbose {
			log.Debugf(log.SubSystemCommMgr, "Slack: %s [%s] connected to %s [%s] \nWebsocket URL: %s.\n",
				s.Details.Self.Name,
				s.Details.Self.ID,
...
func (s *Slack) handlePresenceChange(resp []byte) error {
	...
	if s.Verbose {
		log.Debugf(log.SubSystemCommMgr, "Slack: Presence change. User %s [%s] changed status to %s\n",
...
func (s *Slack) handleMessageResponse(resp []byte, data WebsocketResponse) error {
	...
	if s.Verbose {
		log.Debugf(log.SubSystemCommMgr, "Slack: Message received by %s [%s] with text: %s\n",
...
func (s *Slack) handleHelloResponse() {
	if s.Verbose {
		log.Debugln(log.SubSystemCommMgr, "Slack: Websocket connected successfully.")
	}
	s.Connected = true
...
func (s *Slack) handleReconnectResponse(resp []byte) error {
	...
	if s.Verbose {
		log.Debugf(log.SubSystemCommMgr, "Slack: Reconnect URL set to %s\n", s.ReconnectURL)
	}
	return nil
}
