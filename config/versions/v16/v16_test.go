...
func TestExchanges(t *testing.T) {
	t.Parallel()
	assert.Equal(t, []string{"Huobi", "HTX"}, new(v16.Version).Exchanges(), "Exchanges should return migrated exchange names")
}
...
func TestUpgradeExchange(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			out, err := new(v16.Version).UpgradeExchange(t.Context(), []byte(`{"name":"`+tt.in+`"}`))
	...
	t.Run("preserve unrelated bytes", func(t *testing.T) {
		...
		out, err := new(v16.Version).UpgradeExchange(t.Context(), input)
...
func TestDowngradeExchange(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			out, err := new(v16.Version).DowngradeExchange(t.Context(), []byte(`{"name":"`+tt.in+`"}`))
...
func TestRegisteredUpgrade(t *testing.T) {
	t.Parallel()
	input := []byte(`{"version":11,"exchanges":[{"name":"EXMO"},{"name":"BitMEX"},{"name":"Huobi","enabled":true},{"name":"Kraken","enabled":true}]}`)
	out, err := versions.Manager.Deploy(t.Context(), input, 16)
	require.NoError(t, err, "Deploy must apply the registered v12 through v16 upgrades")
	var config struct {
	...
	assert.Equal(t, uint64(16), config.Version, "Deploy should update the config version")
	require.Len(t, config.Exchanges, 2, "Deploy must remove EXMO and BitMEX")
