...
func TestExchanges(t *testing.T) {
	t.Parallel()
	assert.Equal(t, []string{"Gemini"}, new(v16.Version).Exchanges(), "Exchanges should target Gemini")
}
...
func TestDowngradeExchange(t *testing.T) {
	...
	out, err := new(v16.Version).DowngradeExchange(t.Context(), input)
...
func TestRegisteredUpgrade(t *testing.T) {
	t.Parallel()
	out, err := versions.Manager.Deploy(t.Context(), []byte(`{"version":13,"exchanges":[{"name":"Gemini","api":{"urlEndpoints":{"WebsocketSpotURL":"wss://api.gemini.com"}}}]}`), 16)
	require.NoError(t, err, "Deploy must apply the registered upgrade")
	assert.JSONEq(t, `{"version":16,"exchanges":[{"name":"Gemini","api":{"urlEndpoints":{"WebsocketSpotURL":"wss://api.gemini.com/v2/marketdata"}}}]}`, string(out), "Deploy should update the legacy endpoint")
}
