...
func TestDeploy(t *testing.T) {
	...
	require.ErrorIs(t, err, common.ErrSettingField, "Should throw the correct error trying to add version to bad json")
	require.ErrorContains(t, err, "version", "Should throw the correct error trying to add version to bad json")

	_, err = m.Deploy(context.Background(), []byte(`{"version":"not an int"}`))
	require.ErrorIs(t, err, common.ErrGettingField, "Should throw the correct error trying to get version from bad json")
