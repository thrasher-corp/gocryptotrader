...
func (c *Coinmarketcap) GetCryptocurrencyInfo(currencyID ...uint64) (CryptoCurrencyInfo, error) {
	resp := struct {
	...
	for i := range currencyID {
		currStr[i] = strconv.FormatUint(currencyID[i], 10)
	}
...
func (c *Coinmarketcap) GetCryptocurrencyLatestListing(start, limit uint64) ([]CryptocurrencyLatestListings, error) {
	resp := struct {
	...
	val := url.Values{}
	if start >= 1 {
		val.Set("start", strconv.FormatUint(start, 10))
	}

	if limit > 0 {
		val.Set("limit", strconv.FormatUint(limit, 10))
	}
...
// Start - optionally offsets the paginated items
// limit - optionally sets return limit on items [1..5000]
func (c *Coinmarketcap) GetCryptocurrencyLatestMarketPairs(currencyID, start, limit uint64) (CryptocurrencyLatestMarketPairs, error) {
	resp := struct {
	...
	val := url.Values{}
	val.Set("id", strconv.FormatUint(currencyID, 10))
...
func (c *Coinmarketcap) GetCryptocurrencyOHLCHistorical(currencyID uint64, tStart, tEnd time.Time) (CryptocurrencyOHLCHistorical, error) {
	resp := struct {
...
//
// currencyID - refers to the coinmarketcap currency id
func (c *Coinmarketcap) GetCryptocurrencyOHLCLatest(currencyID uint64) (CryptocurrencyOHLCLatest, error) {
	resp := struct {
...
//
// currencyID - refers to the coinmarketcap currency id
func (c *Coinmarketcap) GetCryptocurrencyLatestQuotes(currencyID ...uint64) (CryptocurrencyLatestQuotes, error) {
	resp := struct {
...
// tStart - refers to the start time of historic value
// tEnd - refers to the end of the time block if zero will default to time.Now()
func (c *Coinmarketcap) GetCryptocurrencyHistoricalQuotes(currencyID uint64, tStart, tEnd time.Time) (CryptocurrencyHistoricalQuotes, error) {
	resp := struct {
	...
	result, found := resp.Data[strconv.FormatUint(currencyID, 10)]
	if !found {
...
func (c *Coinmarketcap) GetExchangeInfo(exchangeID ...uint64) (ExchangeInfo, error) {
	resp := struct {
	...
	for x := range exchangeID {
		exchStr[x] = strconv.FormatUint(exchangeID[x], 10)
	}
...
// Start - optionally offsets the paginated items
// limit - optionally sets return limit on items [1..5000]
func (c *Coinmarketcap) GetExchangeMap(start, limit uint64) ([]ExchangeMap, error) {
	resp := struct {
...
// Start - optionally offsets the paginated items
// limit - optionally sets return limit on items [1..5000]
func (c *Coinmarketcap) GetExchangeLatestMarketPairs(exchangeID, start, limit uint64) (ExchangeLatestMarketPairs, error) {
	resp := struct {
	...
	val := url.Values{}
	val.Set("id", strconv.FormatUint(exchangeID, 10))
...
//
// exchangeID - refers to coinmarketcap exchange id
func (c *Coinmarketcap) GetExchangeLatestQuotes(exchangeID ...uint64) (ExchangeLatestQuotes, error) {
...
// tStart - refers to the start time of historic value
// tEnd - refers to the end of the time block if zero will default to time.Now()
func (c *Coinmarketcap) GetExchangeHistoricalQuotes(exchangeID uint64, tStart, tEnd time.Time) (ExchangeHistoricalQuotes, error) {
	resp := struct {
	...
	result, found := resp.Data[strconv.FormatUint(exchangeID, 10)]
	if !found {
...
// conversion.
func (c *Coinmarketcap) GetPriceConversion(amount float64, currencyID uint64, atHistoricTime time.Time) (PriceConversion, error) {
	resp := struct {
