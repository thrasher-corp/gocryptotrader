...
// system is an overarching type that keeps track of and updates currency,
// currency exchange rates and pairs
var system System

// System contains the loaded system currencies supported on available crypto or
// fiat marketplaces
// NOTE: All internal currencies are upper case
type System struct {
	// FiatCurrencies defines the running fiat currencies in the currency system
	...
	// contract being tracked by the currency system
...
// SetDefaults sets system defaults for basic package functionality
func (s *System) SetDefaults() {
	...
	s.fiatExchangeMarkets = forexprovider.NewDefaultFXProvider() // NOTE: HELP
}
...
func (s *System) RunUpdater(overrides BotOverrides, settings MainConfiguration, filePath string, verbose bool) error {
	if !settings.Cryptocurrencies.HasData() {
		return errors.New("currency system error, no cryptocurrencies loaded")
	}
	s.cryptocurrencies = settings.Cryptocurrencies

	if settings.FiatDisplayCurrency.IsEmpty() {
		return errors.New("currency system error, no fiat display currency set in config")
	}
	...
	if settings.CryptocurrencyProvider.Enabled || overrides.Coinmarketcap {
		...
		// IMPLEMENT LATER
		// err = s.SeedExchangeMarketData(settings.CryptocurrencyProvider)
		// if err != nil {
		// 	log.Warnf("failure seeding exchange market data %s", err)
		// } else {
		// 	if verbose {
		// 		log.Debugf("total market exchanges seeded from Coinmarketcap")
		// 	}
		// }
	} else {
...
func (s *System) SetupConversionRates() {
...
// to the running list
func (s *System) SetDefaultFiatCurrencies(c ...Code) {
...
func (s *System) SetDefaultCryptocurrencies(c ...Code) {
	for _, currency := range c {
...
func (s *System) SetupForexProviders(setting ...base.Settings) {
...
func (s *System) ForeignExchangeUpdater() {
...
// currencies supplied
func (s *System) SeedForeignExchangeRatesByCurrencies(c Currencies) error {
...
func (s *System) SeedForeignExchangeRate(from, to Code) (map[string]float64, error) {
...
func (s *System) GetDefaultForeignExchangeRates() (Conversions, error) {
	if !s.updaterRunning {
...
func (s *System) SeedDefaultForeignExchangeRates() error {
...
// GetSystemExchangeRates returns system seeded exchange rates
func (s *System) GetSystemExchangeRates() (Conversions, error) {
	if !s.updaterRunning {
		err := s.SeedForeignExchangeRates()
		if err != nil {
			return nil, err
...
// SeedForeignExchangeRates seeds the foreign exchange rates from system config
// currencies
func (s *System) SeedForeignExchangeRates() error {
	rates, err := s.fiatExchangeMarkets.GetCurrencyData(
...
func (s *System) updateExchangeRates(m map[string]float64) error {
...
func (s *System) SetupCryptoProvider(settings coinmarketcap.Settings) error {
	if settings.APIkey == "" ||
		settings.APIkey == "key" ||
...
func (s *System) SeedCryptocurrencyMarketData(settings coinmarketcap.Settings) error {
	if !settings.Enabled {
...
// cryptocurrencies
func (s *System) GetTotalMarketCryptocurrencies() (Currencies, error) {
...
// func (s *System) SeedExchangeMarketData(settings coinmarketcap.Settings) error {
// 	if !settings.Enabled {
// 		return errors.New("not enabled please set in config.json with apikey and account levels")
// 	}
...
// 	exchangeData, err := s.CurrencyAnalysis.GetExchangeMap(0, 0)
// 	if err != nil {
// 		return err
// 	}

// 	for _, data := range exchangeData {
// 		s.MarketExchanges = append(s.MarketExchanges, AnalysisData{
// 			ID:          data.ID,
// 			Name:        data.Name,
// 			Slug:        data.Slug,
// 			Active:      data.IsActive == 1,
...
// func (s *System) GetTotalMarketExchanges() []Data {
// 	return s.MarketExchanges
// }

// IsDefaultCurrency returns if a currency is a default currency
func (s *System) IsDefaultCurrency(c Code) bool {
...
// cryptocurrency
func (s *System) IsDefaultCryptocurrency(c Code) bool {
	t, _ := GetTranslation(c)
...
// list
func (s *System) IsFiatCurrency(c Code) bool {
	t, _ := GetTranslation(c)
...
// cryptocurrency list
func (s *System) IsCryptocurrency(c Code) bool {
	t, _ := GetTranslation(c)
...
// currency list
func (s *System) UpdateEnabledFiatCurrencies(c Currencies) {
	for _, i := range c {
...
func (s *System) ExtractBaseCurrencyFromFXResp() Code {
	return s.fxRates.ExtractBase()
}

// ConvertCurrency for example converts $1 USD to the equivalent Japanese Yen
// or vice versa.
func (s *System) ConvertCurrency(amount float64, from, to Code) (float64, error) {
...
// GetSystemRate returns the rate of the conversion value
func (s *System) GetSystemRate(from, to Code) (float64, error) {
	if !s.fxRates.HasData() {
		err := s.SeedDefaultForeignExchangeRates()
		if err != nil {
			return 0, err
...
func (s *System) NewConversion(from, to Code) (Conversion, error) {
	if !s.fxRates.HasData() {
		err := system.SeedDefaultForeignExchangeRates()
		if err != nil {
...
// IsVerbose returns if the system is in verbose mode
func (s *System) IsVerbose() bool {
	return s.Verbose
}
