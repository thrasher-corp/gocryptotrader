...
func TestSetup(t *testing.T) {
	...
	if err != nil {
		t.Fatal("test failed - getting config error", err)
	}
	m = NewManager(cfg.GetCurrencyConfig())
	log.Println("Fiat", m.FiatCurrencies)
	log.Println("Crypto", m.Cryptocurrencies)
}

func TestSeedCurrencyData(t *testing.T) {
	m.SeedCurrencyData()
	if len(m.FXRates) == 0 {
		t.Error("test failed - currency FetchCurrencyData() error")
	}
}
