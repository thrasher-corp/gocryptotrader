...
func main() {
	...
	flag.IntVar(&settings.SyncWorkersCount, "syncworkers", synchronize.DefaultWorkers, "the amount of workers (goroutines) to use for syncing exchange data")
	...
	flag.DurationVar(&settings.SyncTimeoutREST, "synctimeoutrest", synchronize.DefaultTimeoutREST,
		"the amount of time before the syncer will switch from rest protocol to the streaming protocol (e.g. from REST to websocket)")
	flag.DurationVar(&settings.SyncTimeoutWebsocket, "synctimeoutwebsocket", synchronize.DefaultTimeoutWebsocket,
