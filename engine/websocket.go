...
func (c *WebsocketClient) SendWebsocketMessage(evt interface{}) error {
	...
	if err != nil {
		log.Errorf(log.SubSystemWsocMgr, "websocket: failed to send message: %s\n", err)
		return err
	}
	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure) {
				log.Errorf(log.SubSystemWsocMgr, "websocket: client disconnected, err: %s\n", err)
			}
			break
		}
		...
		if msgType == websocket.TextMessage {
			...
			if err != nil {
				log.Errorf(log.SubSystemWsocMgr, "websocket: failed to decode JSON sent from client %s\n", err)
				continue
			}

			if evt.Event == "" {
				log.Warnln(log.SubSystemWsocMgr, "websocket: client sent a blank event, disconnecting")
				continue
			}

			dataJSON, err := common.JSONEncode(evt.Data)
			if err != nil {
				log.Errorln(log.SubSystemWsocMgr, "websocket: client sent data we couldn't JSON decode")
				break
			}

			req := strings.ToLower(evt.Event)
			log.Debugf(log.SubSystemWsocMgr, "websocket: request received: %s\n", req)
			...
			if result.authRequired && !c.Authenticated {
				log.Warnf(log.SubSystemWsocMgr, "Websocket: request %s failed due to unauthenticated request on an authenticated API\n", evt.Event)
			...
			if err != nil {
				log.Errorf(log.SubSystemWsocMgr, "websocket: request %s failed. Error %s\n", evt.Event, err)
				continue
			}
		}
	}
...
func (c *WebsocketClient) write() {
	...
	for { // nolint: gosimple
		...
		case message, ok := <-c.Send:
			...
			if err != nil {
				log.Errorf(log.SubSystemWsocMgr, "websocket: failed to create new io.writeCloser: %s\n", err)
				return
			}
			w.Write(message)
			...
			if err := w.Close(); err != nil {
				log.Errorf(log.SubSystemWsocMgr, "websocket: failed to close io.WriteCloser: %s\n", err)
				return
			}
		}
	}
...
func wsAuth(client *WebsocketClient, data interface{}) error {
	...
	if auth.Username == Bot.Config.RemoteControl.Username && auth.Password == hashPW {
		...
		log.Debugln(log.SubSystemWsocMgr,
