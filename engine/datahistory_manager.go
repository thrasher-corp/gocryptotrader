...
func (m *DataHistoryManager) Start() error {
	...
	m.run()
	log.DataHistory.Debugf("Data history manager %v", MsgSubSystemStarted)
...
func (m *DataHistoryManager) Stop() error {
	...
	close(m.shutdown)
	log.DataHistory.Debugf("Data history manager %v", MsgSubSystemShutdown)
	return nil
}
...
func (m *DataHistoryManager) runJobs() error {
	...
	m.jobs = validJobs
	log.DataHistory.Infof("processing data history jobs")
	for i := 0; (i < int(m.maxJobsPerCycle) || m.maxJobsPerCycle == -1) && i < len(m.jobs); i++ {
		...
		if m.verbose {
			log.DataHistory.Debugf("completed run of data history job %v", m.jobs[i].Nickname)
		}
	}
	log.DataHistory.Infof("completed run of data history jobs")
...
func (m *DataHistoryManager) runJob(job *DataHistoryJob) error {
	...
	if m.verbose {
		log.DataHistory.Debugf("running data history job %v start: %s end: %s interval: %s datatype: %s",
			job.Nickname,
			job.StartDate,
			job.EndDate,
			job.Interval,
...
ranges:
	for i := range job.rangeHolder.Ranges {
		...
		if m.verbose {
			log.DataHistory.Debugf("job %s processing range %v-%v", job.Nickname, job.rangeHolder.Ranges[i].Start, job.rangeHolder.Ranges[i].End)
		}
		intervalsProcessed++
		id, err := uuid.NewV4()
		if err != nil {
...
completionCheck:
	...
	if completed {
		...
		log.DataHistory.Infof("job %s finished! Status: %s", job.Nickname, job.Status)
	}
...
func (m *DataHistoryManager) validateJob(job *DataHistoryJob) error {
	...
	if job.RunBatchLimit <= 0 {
		log.DataHistory.Warnf("job %s has unset batch limit, defaulting to %v", job.Nickname, defaultDataHistoryBatchLimit)
		job.RunBatchLimit = defaultDataHistoryBatchLimit
	}
	if job.MaxRetryAttempts <= 0 {
		log.DataHistory.Warnf("job %s has unset max retry limit, defaulting to %v", job.Nickname, defaultDataHistoryRetryAttempts)
	...
	if job.DataType == dataHistoryTradeDataType &&
		...
		log.DataHistory.Warnf("job %s interval %v outside limits, defaulting to %v", job.Nickname, job.Interval.Word(), defaultDataHistoryTradeInterval)
...
func (m *DataHistoryManager) DeleteJob(nickname, id string) error {
	...
	log.DataHistory.Infof("deleted job %v", dbJob.Nickname)
	return nil
}
