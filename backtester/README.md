...
- The `.strat` config file will contain
  ...
  - Whether to use trade or candle data ([kline data documentation](../backtester/data/kline/README.md))
...
- The exchange order event handler will size to the candle data, run a [slippage estimator](../backtester/eventhandlers/exchange/slippage/README.md), and [place the order](../backtester/eventhandlers/exchange/README.md)
- Upon an order being placed, the order is snapshot for analysis in both the [statistics package](../backtester/eventhandlers/statistics/README.md) and the [report package](../backtester/report/README.md)
...
There is a config generating helper application under `/backtester/config/strategyconfigbuilder` to help you create a `.strat` file. Read the [config builder documentation](../backtester/config/strategyconfigbuilder/README.md). There are also a number of tests under `/backtester/config/strategyconfig_test.go` which generate configs into the `strategyexamples` folder, which if you have code knowledge, can write your own configs programmatically.

### How do I create my own strategy?

Creating strategies requires programming skills. Start with the [strategy documentation](../backtester/eventhandlers/strategies/README.md), then review the [example strategies](../backtester/eventhandlers/strategies/) to understand how to write your own.
