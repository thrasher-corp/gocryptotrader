...
- The `.strat` config file will contain
  ...
  - See [readme](/config/README.md) for a breakdown of all config features
- The GoCryptoTrader Backtester will retrieve the data specified in the config ([readme](/backtest/README.md))
- The data is converted into candles and each candle is streamed as a data event.
- The data event is analysed by the strategy which will output a purchasing signal such as `BUY`, `SELL` or `DONOTHING` ([readme](/eventtypes/signal))
- The purchase signal is then processed by the portfolio manager ([readme](/eventhandlers/portfolio/README.md)) which will size the order ([readme](/eventhandlers/portfolio/size/README.md)) and assess risk ([readme](/eventhandlers/portfolio/risk/README.md)) before sending it to the exchange
- The exchange order event handler will size to the candle data and run a slippage estimator ([readme](/eventhandlers/exchange/slippage/README.md) and place the order ([readme](/eventhandlers/exchange))
- Upon an order being placed, the order is snapshot for analysis in both the statistics package ([readme](/eventhandlers/statistics/README.md)) and the report package ([readme](/report/README.md))
