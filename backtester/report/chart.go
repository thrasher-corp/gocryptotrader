...
// over time
func createPNLCharts(items map[key.ExchangePairAssetKey]*statistics.CurrencyPairStatistic) (*Chart, error) {
	if items == nil {
...
// over time
func createFuturesSpotDiffChart(items map[key.ExchangePairAssetKey]*statistics.CurrencyPairStatistic) (*Chart, error) {
	if items == nil {
		return nil, fmt.Errorf("%w missing currency pair statistics", gctcommon.ErrNilPointer)
	}
